...
async function validateEnvironment() {
  ...
  try {
    ...
    if (!fs.existsSync(envConfigPath)) {
      log('warning', 'Environment configuration file not found (this is normal for most projects)');
    }
    ...
    if (process.env.NODE_ENV === 'production') {
      envFilePath = fs.existsSync(envProductionPath) ? envProductionPath : envLocalPath;
    }
    
    if (!fs.existsSync(envFilePath)) {
      log('warning', `Environment file not found: ${path.basename(envFilePath)}`);
      log('info', 'Create environment file by copying .env.example:');
      log('info', colorize('cyan', `cp .env.example ${path.basename(envFilePath)}`));
      log('info', '');
    } else {
      log('success', `Environment file found: ${path.basename(envFilePath)}`);
    }
    ...
    const checks = [
      ...
      },
      {
        ...
        check: () => {
          ...
          if (key.includes('your-unsplash') || key.includes('placeholder')) {
            return { passed: false, message: '(❌ Contains placeholder value)', level: 'error' };
          }
      ...
      },
      {
        ...
        check: () => {
          ...
          if (key.includes('your-openai') || key.includes('placeholder')) {
            return { passed: false, message: '(❌ Contains placeholder value)', level: 'error' };
          }
      ...
      },
      {
        ...
        check: () => {
          ...
          if (url.includes('your-supabase') || key.includes('your-supabase')) {
            return { passed: false, message: '(❌ Contains placeholder values)', level: 'error' };
          }
    ...
    if (hasErrors) {
      ...
      if (process.env.NODE_ENV === 'production') {
        log('info', '\n' + colorize('red', 'Production Security Checklist:'));
        log('info', '• Generate secure keys: node -e "console.log(require(\'crypto\').randomBytes(32).toString(\'hex\'))"');
        log('info', '• Set ALLOWED_ORIGINS to your actual production domains');
        log('info', '• Disable DEBUG_ENDPOINT_ENABLED');
        log('info', '• Configure appropriate rate limiting');
        log('info', '• Replace all placeholder API keys with real production keys');
      }
    ...
    if (!process.env.OPENAI_API_KEY || !process.env.NEXT_PUBLIC_UNSPLASH_ACCESS_KEY) {
      ...
      log('info', '• Add real API keys to environment file to disable demo mode');
    }
    
    // Production-specific recommendations
    if (process.env.NODE_ENV === 'production') {
      log('info', '\n' + colorize('magenta', 'Production Recommendations:'));
      log('info', '• Enable monitoring and error reporting (Sentry)');
      log('info', '• Set up caching (Redis/KV)');
      log('info', '• Enable security headers');
      log('info', '• Configure SSL/HTTPS');
      log('info', '• Set up backup and recovery procedures');
    }
