...
CREATE INDEX IF NOT EXISTS idx_users_spanish_level ON users(spanish_level);
...
CREATE INDEX IF NOT EXISTS idx_images_unsplash_id ON images(unsplash_id) WHERE unsplash_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_images_usage_count ON images(usage_count);
CREATE INDEX IF NOT EXISTS idx_images_suitable_learning ON images(is_suitable_for_learning);
CREATE INDEX IF NOT EXISTS idx_images_content_rating ON images(content_rating);

-- Vocabulary indexes
CREATE INDEX IF NOT EXISTS idx_vocabulary_lists_created_by ON vocabulary_lists(created_by);
CREATE INDEX IF NOT EXISTS idx_vocabulary_lists_category ON vocabulary_lists(category);
CREATE INDEX IF NOT EXISTS idx_vocabulary_lists_active ON vocabulary_lists(is_active);
CREATE INDEX IF NOT EXISTS idx_vocabulary_items_list_id ON vocabulary_items(vocabulary_list_id);
CREATE INDEX IF NOT EXISTS idx_vocabulary_items_difficulty ON vocabulary_items(difficulty_level);
CREATE INDEX IF NOT EXISTS idx_vocabulary_items_pos ON vocabulary_items(part_of_speech);
CREATE INDEX IF NOT EXISTS idx_vocabulary_items_frequency ON vocabulary_items(frequency_score);
CREATE INDEX IF NOT EXISTS idx_vocabulary_items_spanish_text ON vocabulary_items USING gin(to_tsvector('spanish', spanish_text));
CREATE INDEX IF NOT EXISTS idx_vocabulary_items_english_translation ON vocabulary_items USING gin(to_tsvector('english', english_translation));

-- Learning progress indexes
CREATE INDEX IF NOT EXISTS idx_learning_progress_user_id ON learning_progress(user_id);
CREATE INDEX IF NOT EXISTS idx_learning_progress_vocab_id ON learning_progress(vocabulary_item_id);
CREATE INDEX IF NOT EXISTS idx_learning_progress_user_vocab ON learning_progress(user_id, vocabulary_item_id);
CREATE INDEX IF NOT EXISTS idx_learning_progress_phase ON learning_progress(learning_phase);
CREATE INDEX IF NOT EXISTS idx_learning_progress_next_review ON learning_progress(next_review) WHERE next_review IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_learning_progress_mastery ON learning_progress(mastery_level);

-- Content table indexes
CREATE INDEX IF NOT EXISTS idx_saved_descriptions_user_id ON saved_descriptions(user_id);
CREATE INDEX IF NOT EXISTS idx_saved_descriptions_image_id ON saved_descriptions(image_id);
CREATE INDEX IF NOT EXISTS idx_saved_descriptions_style ON saved_descriptions(description_style);
CREATE INDEX IF NOT EXISTS idx_saved_descriptions_favorite ON saved_descriptions(is_favorite) WHERE is_favorite = true;
CREATE INDEX IF NOT EXISTS idx_saved_descriptions_public ON saved_descriptions(is_public) WHERE is_public = true;
CREATE INDEX IF NOT EXISTS idx_saved_descriptions_created_at ON saved_descriptions(created_at);

-- Q&A indexes
CREATE INDEX IF NOT EXISTS idx_qa_responses_user_id ON qa_responses(user_id);
CREATE INDEX IF NOT EXISTS idx_qa_responses_image_id ON qa_responses(image_id);
CREATE INDEX IF NOT EXISTS idx_qa_responses_description_id ON qa_responses(description_id);
CREATE INDEX IF NOT EXISTS idx_qa_responses_difficulty ON qa_responses(difficulty);
CREATE INDEX IF NOT EXISTS idx_qa_responses_correct ON qa_responses(is_correct) WHERE is_correct IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_user_interactions_type_action ON user_interactions(interaction_type, action);
...
CREATE INDEX IF NOT EXISTS idx_learning_analytics_user_date ON learning_analytics(user_id, date_recorded);

-- Composite indexes for common queries
CREATE INDEX IF NOT EXISTS idx_sessions_user_type_date ON sessions(user_id, session_type, started_at);
CREATE INDEX IF NOT EXISTS idx_vocabulary_list_difficulty_active ON vocabulary_items(vocabulary_list_id, difficulty_level) WHERE vocabulary_list_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_progress_user_phase_review ON learning_progress(user_id, learning_phase, next_review);
...
DROP TRIGGER IF EXISTS update_vocabulary_lists_updated_at ON vocabulary_lists;
...
DROP TRIGGER IF EXISTS update_saved_descriptions_updated_at ON saved_descriptions;
...
DROP FUNCTION IF EXISTS update_vocabulary_list_stats() CASCADE;
...
DROP TRIGGER IF EXISTS update_vocabulary_list_stats_trigger ON vocabulary_items;
...
DROP TRIGGER IF EXISTS calculate_session_duration_trigger ON sessions;
...
DROP POLICY IF EXISTS "Users can view accessible vocabulary lists" ON vocabulary_lists;
...
DROP POLICY IF EXISTS "Users can view accessible descriptions" ON saved_descriptions;
...
DROP POLICY IF EXISTS "Users can manage own descriptions" ON saved_descriptions;
CREATE POLICY "Users can manage own descriptions" ON saved_descriptions
    FOR ALL USING (user_id = auth.uid() OR user_id IS NULL);

-- QA responses policies
DROP POLICY IF EXISTS "Users can view accessible qa responses" ON qa_responses;
CREATE POLICY "Users can view accessible qa responses" ON qa_responses
    FOR SELECT USING (user_id = auth.uid() OR user_id IS NULL);

DROP POLICY IF EXISTS "Users can manage own qa responses" ON qa_responses;
...
DROP POLICY IF EXISTS "Users can manage own analytics" ON learning_analytics;
...
DROP POLICY IF EXISTS "Users can view vocabulary items from accessible lists" ON vocabulary_items;
...
DROP POLICY IF EXISTS "Users can manage vocabulary items in own lists" ON vocabulary_items;
...
-- Insert default vocabulary lists (only if they don't exist)
...
SELECT
    ...
    0
WHERE NOT EXISTS (
    SELECT 1 FROM vocabulary_lists WHERE name = 'Básico - Primeras Palabras'
);
...
SELECT
    ...
    0
WHERE NOT EXISTS (
    SELECT 1 FROM vocabulary_lists WHERE name = 'Colores y Formas'
);
...
SELECT
    ...
    0
WHERE NOT EXISTS (
    SELECT 1 FROM vocabulary_lists WHERE name = 'Casa y Familia'
);
...
BEGIN
    ...
    RAISE NOTICE 'SAFE IDEMPOTENT VERSION - Can be run multiple times';
    RAISE NOTICE '==============================================';
