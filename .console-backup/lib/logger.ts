...
class Logger {
  ...
  warn(message: string, errorOrContext?: Error | unknown | LogContext, context?: LogContext): void {
    // Handle both warn(message, context) and warn(message, error, context) signatures
    if (errorOrContext && (errorOrContext instanceof Error || typeof errorOrContext === 'string')) {
      const errorContext = {
        ...context,
        error: {
          name: errorOrContext instanceof Error ? errorOrContext.name : 'Warning',
          message: errorOrContext instanceof Error ? errorOrContext.message : String(errorOrContext),
          stack: errorOrContext instanceof Error ? errorOrContext.stack : undefined,
        },
      };
      this.writeLog('warn', message, errorContext);
    } else {
      this.writeLog('warn', message, errorOrContext as LogContext);
    }
  }

  info(message: string, errorOrContext?: unknown | LogContext, context?: LogContext): void {
    // Handle both info(message, context) and info(message, error, context) signatures
    if (errorOrContext && typeof errorOrContext === 'object' && !('userId' in errorOrContext || 'requestId' in errorOrContext)) {
      ...
      this.writeLog('info', message, errorContext);
    } else {
      this.writeLog('info', message, errorOrContext as LogContext);
    }
  }
  ...
  /**
   * Log API call (alias for apiRequest for compatibility)
   */
  apiCall(method: string, url: string, context?: LogContext): void {
    this.apiRequest(method, url, context);
  }
