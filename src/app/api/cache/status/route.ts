...
import { tieredCache, imageCache, descriptionCache, qaCache, phrasesCache } from '@/lib/cache/tiered-cache';
        ...
        detailed: {
          ...
          infrastructure: {
            ...
            environment: {
              ...
              memoryFallbackEnabled: process.env.ENABLE_MEMORY_CACHE_FALLBACK !== 'false'
            }
          }
        }
      })
...
function calculateAverageHitRate(metrics: Array<{ hitRate: number }>): number {
  const validMetrics = metrics.filter(m => typeof m.hitRate === 'number' && !isNaN(m.hitRate));
...
async function performCacheClear(pattern?: string, cacheType?: string): Promise<any> {
