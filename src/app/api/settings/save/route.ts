...
const userSettingsSchema = z.object({
  ...
  settings: z.object({
    ...
    export: z
      .object({
        defaultFormat: z
          .enum(["json", "csv", "txt", "pdf", "anki", "quizlet"])
          .default("json"),
        includeMetadata: z.boolean().default(false),
        includeProgress: z.boolean().default(false),
        autoExportInterval: z
    ...
    if (sections && sections.length > 0) {
      ...
      if (currentSettings) {
        ...
        sections.forEach((section) => {
          if (
            this.defaultSettings[section as keyof typeof this.defaultSettings]
          ) {
            resetSettings[section as keyof typeof resetSettings] =
              this.defaultSettings[
                section as keyof typeof this.defaultSettings
              ];
          }
        });
    ...
    return NextResponse.json(
      {
        ...
        message:
          "An error occurred while retrieving your settings. Please try again.",
        timestamp: new Date().toISOString(),
        retry: true,
      },
      {
    ...
    return NextResponse.json(
      {
        ...
        message:
          "An error occurred while resetting your settings. Please try again.",
        timestamp: new Date().toISOString(),
        retry: true,
      },
      {
