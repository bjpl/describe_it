...
const exportRequestSchema = z.object({
  ...
  formatting: z.object({
    ...
    template: z.enum(['minimal', 'standard', 'detailed']).optional().default('standard')
  }).optional().default({})
});
...
class ExportService {
  ...
  async generateExport(
    ...
    const content = await this.getContentForExport(userId, contentType, filters);
  ...
  async getVocabularyContent(userId: string, filters: any) {
    ...
    if (filters.difficulty) {
      items = items.filter((item: any) => item.difficulty === filters.difficulty);
    }
    if (filters.collectionName) {
      items = items.filter((item: any) => item.collectionName === filters.collectionName);
    }
  ...
  async getPhrasesContent(userId: string, filters: any) {
    ...
    if (filters.category) {
      filteredPhrases = filteredPhrases.filter((phrase: any) => phrase.category === filters.category);
    }
    if (filters.difficulty) {
      filteredPhrases = filteredPhrases.filter((phrase: any) => phrase.difficulty === filters.difficulty);
    }
  ...
  generateCSV(content: any[], formatting: any) {
    if (content.length === 0) {
      return { data: '', filename: `export_${Date.now()}.csv`, contentType: 'text/csv' };
    }
...
<body>
  ...
  getCSVColumns(sampleItem: any, formatting: any): string[] {
    ...
    if (formatting.includeTranslations && sampleItem.translation) optionalColumns.push('translation');
    if (formatting.includeMetadata) optionalColumns.push('createdAt', 'confidence');
  ...
  try {
    ...
    if (exportResult) {
      ...
      exportResult = await exportService.generateExport(
        userId, exportType, contentType, filters, formatting
      );
  ...
  try {
    ...
    return NextResponse.json(
      { 
        ...
        message: 'An error occurred while downloading your export. Please try again.',
        timestamp: new Date().toISOString()
      },
      { status: 500 }
    );
