...
const vocabularyQuerySchema = z.object({
  ...
  sortBy: z.enum(['phrase', 'difficulty', 'createdAt', 'masteryLevel']).optional().default('createdAt'),
...
class VocabularyStorage {
  ...
  async saveVocabulary(userId: string, vocabulary: any, collectionName: string, metadata?: any) {
    ...
    await this.updateCollectionIndex(userId, collectionName, vocabularyId, vocabulary);
  ...
  async saveBulkVocabulary(userId: string, vocabularyItems: any[], collectionName: string, metadata?: any) {
    const results = [];
    const timestamp = new Date().toISOString();
    ...
    for (const vocabulary of vocabularyItems) {
      const vocabularyItem = {
        ...vocabulary,
        id: vocabulary.id || `vocab_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        userId,
        collectionName,
        createdAt: timestamp,
        updatedAt: timestamp,
  ...
  async updateCollectionIndex(userId: string, collectionName: string, vocabularyId: string, vocabulary: any) {
    ...
    try {
      ...
      index.collections[collectionName] = {
        name: collectionName,
        itemCount: index.items.filter((item: any) => item.collectionName === collectionName).length,
        lastUpdated: new Date().toISOString()
      };
  ...
  try {
    ...
    if (body.vocabularyItems && Array.isArray(body.vocabularyItems)) {
      ...
      const results = await vocabularyStorage.saveBulkVocabulary(
        userId, vocabularyItems, collectionName, metadata
      );
      ...
      const result = await vocabularyStorage.saveVocabulary(
        userId, vocabulary, collectionName, metadata
      );
    ...
    return NextResponse.json(
      { 
        ...
        message: 'An error occurred while saving your vocabulary. Please try again.',
        timestamp: new Date().toISOString(),
        retry: true
      },
      { 
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await vocabularyStorage.getVocabulary(filters.userId, filters);
    ...
    return NextResponse.json(
      { 
        ...
        message: 'An error occurred while retrieving your vocabulary. Please try again.',
        timestamp: new Date().toISOString(),
        retry: true
      },
      { 
