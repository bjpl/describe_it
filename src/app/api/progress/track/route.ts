...
const progressEventSchema = z.object({
  ...
  eventType: z.enum([
    'vocabulary_learned', 'vocabulary_reviewed', 'vocabulary_mastered',
    'qa_answered', 'qa_correct', 'qa_incorrect',
    'phrase_learned', 'phrase_reviewed', 'phrase_mastered',
    'translation_completed', 'description_generated',
    'session_started', 'session_completed',
    'image_processed', 'export_generated',
    'difficulty_adjusted', 'goal_achieved'
  ]),
  eventData: z.object({
...
const goalSchema = z.object({
  userId: z.string().optional().default('anonymous'),
  goalType: z.enum(['daily_vocabulary', 'weekly_practice', 'mastery_target', 'streak_goal']),
...
class ProgressTracker {
  ...
  private sessionPrefix = (sessionId: string) => `${this.cachePrefix}:session:${sessionId}`;

  async trackEvent(userId: string, eventType: string, eventData: any, sessionId?: string, timestamp?: string) {
    ...
    if (sessionId) {
      await this.updateSessionProgress(sessionId, eventType, eventData, eventTime);
    }
  ...
  async updateUserProgress(userId: string, eventType: string, eventData: any, timestamp: string) {
    ...
    try {
      ...
      if (eventData.category) {
        progress.categories[eventData.category] = (progress.categories[eventData.category] || 0) + 1;
      }
      ...
      if (['vocabulary_learned', 'qa_correct', 'phrase_mastered'].includes(eventType)) {
        const today = timestamp.split('T')[0];
        const lastActivityDate = progress.lastActivity.split('T')[0];
        
        if (today === lastActivityDate || this.isConsecutiveDay(lastActivityDate, today)) {
  ...
  async updateSessionProgress(sessionId: string, eventType: string, eventData: any, timestamp: string) {
  ...
  async updateDailyAggregation(userId: string, timestamp: string, eventType: string, eventData: any) {
    ...
    try {
      ...
      if (eventData.category) {
        daily.categories[eventData.category] = (daily.categories[eventData.category] || 0) + 1;
      }
      ...
      for (const goal of goals.active) {
        ...
        switch (goal.goalType) {
          ...
          case 'weekly_practice':
            if (['vocabulary_learned', 'qa_answered', 'phrase_learned'].includes(eventType)) increment = 1;
            break;
          case 'mastery_target':
            if (eventType === 'vocabulary_mastered' || eventType === 'phrase_mastered') increment = 1;
            break;
          case 'streak_goal':
            if (['vocabulary_learned', 'qa_correct'].includes(eventType)) increment = 1;
            break;
        }
  ...
  checkAchievements(progress: any, eventType: string, eventData: any) {
    const achievements = [
      { id: 'first_word', condition: () => progress.totalEvents === 1, title: 'First Steps' },
      { id: 'vocab_100', condition: () => progress.categories.vocabulary >= 100, title: 'Vocabulary Master' },
      { id: 'streak_7', condition: () => progress.streaks.current >= 7, title: 'Week Warrior' },
      { id: 'streak_30', condition: () => progress.streaks.current >= 30, title: 'Monthly Master' },
      { id: 'perfect_score', condition: () => progress.scores.average >= 0.95, title: 'Perfectionist' }
    ];

    achievements.forEach(achievement => {
      if (achievement.condition() && !progress.achievements.find((a: any) => a.id === achievement.id)) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const event = await progressTracker.trackEvent(
      userId, eventType, eventData, sessionId, timestamp
    );
    ...
    return NextResponse.json(
      { 
        ...
        message: 'An error occurred while tracking your progress. Please try again.',
        timestamp: new Date().toISOString(),
        retry: true
      },
      { 
  ...
  try {
    ...
    return NextResponse.json(
      { 
        ...
        message: 'An error occurred while retrieving your progress. Please try again.',
        timestamp: new Date().toISOString(),
        retry: true
      },
      { 
