  ...
  const generateDescriptions = async () => {
    ...
    try {
      ...
      if (!englishResponse.ok || !spanishResponse.ok) {
        throw new Error(
          `API Error: ${englishResponse.status} / ${spanishResponse.status}`,
        );
      }
      ...
      if (
        (englishData.success || englishData.metadata?.fallback) &&
        (spanishData.success || spanishData.metadata?.fallback)
      ) {
        setGeneratedDescriptions({
          english:
            englishData.data?.text ||
            "An interesting image that showcases unique visual elements with colors and composition that create an engaging visual experience.",
          spanish:
            spanishData.data?.text ||
            "Una imagen interesante que muestra elementos visuales únicos con colores y composición que crean una experiencia visual atractiva.",
        });
      } else {
        // Fallback to demo descriptions
        setGeneratedDescriptions({
          english:
            "This image displays fascinating visual elements with vibrant colors and thoughtful composition that invites contemplation and learning.",
          spanish:
            "Esta imagen muestra elementos visuales fascinantes con colores vibrantes y una composición reflexiva que invita a la contemplación y el aprendizaje.",
        });
      }
    } catch (error) {
      ...
      setGeneratedDescriptions({
        english:
          "This is an interesting image with visual elements that provide learning opportunities. The composition and details offer educational value for language learning.",
        spanish:
          "Esta es una imagen interesante con elementos visuales que brindan oportunidades de aprendizaje. La composición y los detalles ofrecen valor educativo para el aprendizaje de idiomas.",
      });
      setDescriptionError(
        "Using demo descriptions. API may be temporarily unavailable.",
      );
    } finally {
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-2">
              <button
                ...
                title={
                  !selectedImage || !generatedDescriptions.english
                    ? "Generate descriptions first to export"
                    : "Export data"
                }
              >
                    ...
                    {images.slice(0, 4).map((image, index) => (
                      ...
                      >
                        <img
                          ...
                          decoding="async"
                          onLoad={() =>
                            console.debug("Image loaded:", image.id)
                          }
                          onError={() =>
                            console.warn("Image failed to load:", image.id)
                          }
                        />
                      </button>
                    ))}
            ...
            <Tabs.Root value={activeTab} onValueChange={setActiveTab}>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
                ...
                <Tabs.Content value="qa">
                  ...
                  >
                    <Suspense
                      fallback={<LoadingState message="Loading Q&A panel..." />}
                    >
                      <QAPanel
                ...
                <Tabs.Content value="phrases">
                  ...
                  >
                    <Suspense
                      fallback={
                        <LoadingState message="Loading phrases panel..." />
                      }
                    >
                      <PhrasesPanel
                        selectedImage={selectedImage}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 max-w-md w-full">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-4">
              Describe It is an interactive Spanish learning application that
              uses images to enhance language acquisition.
            </p>
            <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400 mb-6">
              ...
              <p>
                <strong>Status:</strong> Demo Mode (no API keys required)
              </p>
            </div>
            <button
