...
export const DescriptionPanel = memo<DescriptionPanelProps>(
        ...
        {selectedImage && (
          <div className="space-y-4">
              ...
              {styleOptions.map((option) => {
                ...
                return (
                  <button
                    key={option.value}
                    onClick={() =>
                      onStyleChange(option.value as DescriptionStyle)
                    }
                    disabled={isGenerating}
                    className={`
                  ...
                  >
                    <div className="flex items-center gap-2 mb-2">
                      <Icon
                        className={`h-5 w-5 ${isSelected ? option.color : "text-gray-400"}`}
                      />
                      <span
                        className={`font-medium text-sm ${isSelected ? option.color : "text-gray-700 dark:text-gray-300"}`}
                      >
                      ...
                      {isSelected && (
                        <div
                          className={`w-2 h-2 rounded-full ${option.color.replace("text-", "bg-").replace("dark:text-", "dark:bg-")} ml-auto`}
                        />
                      )}
                    </div>
                    <p
                      className={`text-xs ${isSelected ? "text-gray-600 dark:text-gray-400" : "text-gray-500 dark:text-gray-500"}`}
                    >
                      {option.description}
                    </p>
                  </button>
            ...
            {(() => {
              ...
              return (
                <div
                  className={`p-3 rounded-lg ${currentStyle.bgColor} border-l-4 border-current ${currentStyle.color}`}
                >
                  <div className="flex items-center gap-2">
                    <currentStyle.icon
                      className={`h-4 w-4 ${currentStyle.color}`}
                    />
                    <span
                      className={`text-sm font-medium ${currentStyle.color}`}
                    >
        ...
        {selectedImage ? (
          <div className="space-y-4">
            ...
            {(() => {
              ...
              return (
                <div
                  className={`p-4 rounded-lg ${currentStyle?.bgColor || "bg-green-50 dark:bg-green-900/20"}`}
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <Icon
                        className={`h-4 w-4 ${currentStyle?.color || "text-green-600 dark:text-green-400"}`}
                      />
                      <h3
                        className={`font-semibold ${currentStyle?.color || "text-green-900 dark:text-green-300"}`}
                      >
                        Español -{" "}
                        {currentStyle?.label.split(" ")[0] || "Narrativo"}
                      </h3>
                    </div>
                    {generatedDescriptions.spanish && (
                      <span
                        className={`text-xs px-2 py-1 rounded ${
                          currentStyle?.color.includes("blue")
                            ? "bg-blue-200 dark:bg-blue-800 text-blue-800 dark:text-blue-200"
                            : currentStyle?.color.includes("purple")
                              ? "bg-purple-200 dark:bg-purple-800 text-purple-800 dark:text-purple-200"
                              : currentStyle?.color.includes("gray")
                                ? "bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200"
                                : currentStyle?.color.includes("green")
                  ...
                  <div
                    className={`text-gray-700 dark:text-gray-300 ${
                      selectedStyle === "poetico"
                        ? "italic leading-relaxed"
                        : selectedStyle === "academico"
                          ? "leading-relaxed font-light"
                          : selectedStyle === "conversacional"
                            ? "leading-normal"
                            : selectedStyle === "infantil"
