...
type CategoryFilter = 'all' | 'sustantivos' | 'verbos' | 'adjetivos' | 'adverbios' | 'frasesClaves';
...
const EnhancedPhrasesPanel = memo<EnhancedPhrasesPanelProps>(function EnhancedPhrasesPanel({
  selectedImage,
  descriptionText,
  style,
}) {
  const [phrases, setPhrases] = useState<CategorizedPhrase[]>([]);
  const [categorizedPhrases, setCategorizedPhrases] = useState<Record<string, CategorizedPhrase[]>>(
    {}
  );
  ...
  const extractPhrases = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(responseData.message || responseData.error || 'Failed to extract phrases');
      }

      // API returns { phrases: [...], categorizedPhrases: {...}, metadata: {...} }
      const extractedPhrases: CategorizedPhrase[] = responseData.phrases || responseData;

      // Validate that we got an array
      if (!Array.isArray(extractedPhrases)) {
        throw new Error('Invalid response format: expected phrases array');
      }

      setPhrases(extractedPhrases);

      // Use server-side categorization if available, otherwise sort locally
      if (responseData.categorizedPhrases && typeof responseData.categorizedPhrases === 'object') {
        setCategorizedPhrases(responseData.categorizedPhrases);
      } else {
  ...
  const handleDifficultyChange = useCallback(
    (e: React.ChangeEvent<HTMLSelectElement>) => setDifficulty(e.target.value as DifficultyLevel),
    []
  );

  const handleMaxPhrasesChange = useCallback(
    (e: React.ChangeEvent<HTMLSelectElement>) => setMaxPhrases(Number(e.target.value)),
    []
  );
  ...
  const savePhrase = useCallback(async (phrase: CategorizedPhrase) => {
    try {
      ...
      // Update local state
      setPhrases(prev => prev.map(p => (p.id === phrase.id ? updatedPhrase : p)));
      ...
      // FUTURE: Persist saved phrases to backend/localStorage for cross-session persistence
  ...
  // NOTE: Moved above translateAllPhrases to fix React hooks dependency order
    ...
    try {
      const response = await fetch('/api/translate', {
        ...
        body: safeStringify({
          ...
          fromLanguage: 'es',
          toLanguage: 'en',
          context: 'Spanish language learning phrases with educational context',
        }),
      });
      ...
      phrasesToTranslate.forEach(phrase => {
        errorTranslations[phrase.id] = `Translation unavailable for "${phrase.phrase}"`;
      });
  ...
  }, [filteredCategories, phraseTranslations, translatingBatch]);
          ...
          <div className='flex items-start gap-3'>
            ...
            <div>
              ...
              <p className='text-sm text-yellow-700 dark:text-yellow-400 mt-1'>
                Please generate a description first. The phrases will be categorized by grammatical
                type.
              </p>
            </div>
          </div>
  ...
  return (
    <div className='space-y-6'>
          ...
          <div className='flex flex-col sm:flex-row gap-3'>
            ...
            <div className='flex items-center gap-2'>
              ...
              {showTranslations && (
                ...
                >
                  <RefreshCw className={`h-4 w-4 ${translatingBatch ? 'animate-spin' : ''}`} />
        ...
        <div className='space-y-6'>
          <div className='flex items-center justify-between'>
            <p className='text-sm text-gray-600 dark:text-gray-400'>
              Found {phrases.length} phrases in {Object.keys(filteredCategories).length} categories
            </p>
            <span
                  ...
                  {categoryPhrases.map(phrase => (
                    ...
                    >
                      <div className='space-y-3'>
                        ...
                        <div className='flex items-start justify-between'>
                          <div className='flex-1'>
                            ...
                            <p className='text-xs text-gray-500 dark:text-gray-400'>
                              {phrase.partOfSpeech}
                              {phrase.conjugation && ` • Infinitive: ${phrase.conjugation}`}
                            </p>
                          </div>
                          ...
                          <div className='flex items-center gap-2'>
                            ...
                            <button
                              ...
                              title={phrase.saved ? 'Already saved' : 'Save to vocabulary'}
                            >
                              {phrase.saved ? (
                        ...
                        {/* Translation */}
                        {showTranslations && phraseTranslations[phrase.id] && (
            ...
            <div className='flex flex-wrap gap-3'>
              ...
              {showTranslations && (
                <div className='flex items-center gap-2 text-sm text-blue-600 dark:text-blue-400'>
                  <Languages className='h-4 w-4' />
                  <span>
                    {Object.keys(phraseTranslations).length} of {phrases.length} phrases translated
                  </span>
                </div>
              )}
      ...
      {/* Empty State */}
      {phrases.length === 0 && !loading && !error && selectedImage && descriptionText && (
        <div className='text-center py-8'>
          <BookOpen className='h-8 w-8 text-gray-400 mx-auto mb-3' />
          <p className='text-gray-600 dark:text-gray-400 mb-4'>No phrases extracted yet.</p>
          <button
            onClick={extractPhrases}
