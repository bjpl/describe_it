...
import { Download, TrendingUp, Clock, BookOpen, Target, Activity } from 'lucide-react';
import { SessionLogger, getSessionLogger } from '@/lib/logging/sessionLogger';
import { SessionReportGenerator, VisualReportData, ChartData } from '@/lib/logging/sessionReportGenerator';
import { SessionReport as SessionReportType, SessionSummary } from '@/types/session';
...
export function SessionReport({ 
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'charts' | 'analytics' | 'export'>('overview');
    ...
    try {
      ...
      switch (format) {
        ...
        case 'html':
          ...
          break;
        case 'pdf':
          blob = await PDFExporter.exportReportToPDF(reportData, visualData, reportGenerator, {
            includeCharts: true,
    ...
    return (
      <div className={`flex items-center justify-center p-8 ${className}`}>
        <div className="text-center">
          ...
          <p className="text-gray-600">Generating comprehensive session report...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`bg-white rounded-lg shadow-lg overflow-hidden ${className}`}>
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6">
        <div className="flex justify-between items-center">
          ...
          <div className="text-right">
            <div className="text-3xl font-bold">{reportData.summary.learningScore}</div>
      ...
      <div className="p-6">
        ...
        {activeTab === 'charts' && (
          <ChartsTab visualData={visualData} />
        )}
        ...
        <div className="bg-gray-50 rounded-lg p-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{summary.descriptionsGenerated}</span>
            </div>
            <div className="flex justify-between">
        ...
        <div className="bg-gray-50 rounded-lg p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="font-medium capitalize">{summary.comprehensionLevel}</span>
            </div>
            <div className="flex justify-between">
              <span>Progress to Next</span>
              <span className="font-medium">{progressReport.progressToNext}%</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{vocabularyReport.wordDiversity}/6</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-8">
      ...
      <div className="bg-gray-50 rounded-lg p-6">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={[
            { name: 'Learning Score', value: visualData.progressCharts.learningScore.datasets[0].data[0] },
            { name: 'Engagement Score', value: visualData.progressCharts.learningScore.datasets[0].data[1] }
          ]}>
      ...
      <div className="bg-gray-50 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                data={visualData.progressCharts.activityBreakdown.labels.map((label, index) => ({
                  name: label,
                  value: visualData.progressCharts.activityBreakdown.datasets[0].data[index]
                }))}
                cx="50%"
                cy="50%"
                outerRadius={100}
                dataKey="value"
              >
                {visualData.progressCharts.activityBreakdown.labels.map((_, index) => (
          ...
          <div className="flex flex-col justify-center space-y-2">
            {visualData.progressCharts.activityBreakdown.labels.map((label, index) => (
              <div key={index} className="flex items-center space-x-3">
                ...
                <span className="font-medium">
                  {visualData.progressCharts.activityBreakdown.datasets[0].data[index]}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="bg-gray-50 rounded-lg p-6">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart data={visualData.progressCharts.vocabularyGrowth.labels.map((label, index) => ({
            date: label,
            cumulative: visualData.progressCharts.vocabularyGrowth.datasets[0].data[index],
            new: visualData.progressCharts.vocabularyGrowth.datasets[1].data[index]
          }))}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="date" />
            <YAxis />
            ...
            <Area type="monotone" dataKey="new" stackId="2" stroke="#10b981" fill="#10b981" fillOpacity={0.6} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie
                  data={Object.entries(visualData.progressMetrics.timeSpentByActivity).map(([activity, time]) => ({
                  ...
                  dataKey="value"
                >
                  {Object.keys(visualData.progressMetrics.timeSpentByActivity).map((_, index) => (
                    <Cell key={`cell-${index}`} fill={PIE_COLORS[index % PIE_COLORS.length]} />
                  ))}
                </Pie>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-lg">{Math.round(timeAnalysis.totalTime / 1000 / 60)}m</span>
            </div>
            <div className="flex justify-between items-center">
              <span>Active Learning Time</span>
              <span className="font-bold text-lg">{Math.round(timeAnalysis.activeTime / 1000 / 60)}m</span>
            </div>
            <div className="flex justify-between items-center">
              <span>Efficiency Score</span>
              <span className="font-bold text-lg" style={{ color: timeAnalysis.efficiencyScore >= 70 ? '#22c55e' : '#f59e0b' }}>
      ...
      <div className="bg-gray-50 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="text-center">
            <div className="text-3xl font-bold mb-2" style={{ color: errorAnalysis.errorRate < 5 ? '#22c55e' : '#ef4444' }}>
              {errorAnalysis.errorRate.toFixed(1)}%
            </div>
            <div className="text-gray-600">Error Rate</div>
          </div>
          <div className="text-center">
            <div className="text-3xl font-bold mb-2 capitalize" style={{ 
              color: errorAnalysis.stability === 'excellent' ? '#22c55e' : 
                    errorAnalysis.stability === 'good' ? '#3b82f6' :
                    errorAnalysis.stability === 'fair' ? '#f59e0b' : '#ef4444'
            }}>
          ...
          <div className="text-center">
            <div className="text-3xl font-bold mb-2">{errorAnalysis.commonErrors.length}</div>
...
function ExportTab({ 
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="font-semibold text-lg mb-2">Export Your Session Report</h3>
        <p className="text-gray-600 mb-6">
          Choose your preferred format to download and save your learning session data.
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {exportOptions.map((option) => (
          <div key={option.format} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
            <div className="flex items-start space-x-4">
              <div className={`${option.color} text-white p-3 rounded-lg text-xl`}>
                {option.icon}
              </div>
      ...
      <div className="bg-gray-50 rounded-lg p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          <div>
            <div className="text-2xl font-bold text-blue-600">{reportData.summary.totalInteractions}</div>
            <div className="text-sm text-gray-600">Total Interactions</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-green-600">{reportData.summary.learningScore}</div>
            <div className="text-sm text-gray-600">Learning Score</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-purple-600">{reportData.summary.vocabularySelected}</div>
