...
const QAPanel = memo<QAPanelProps>(function QAPanel({
  ...
  const generateQuestions = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < questionPrompts.length; i++) {
        ...
        const question = createMultipleChoiceFromAnswer(
          qaData,
          questionPrompts[i],
        );
  ...
  const createMultipleChoiceFromAnswer = useCallback(
    (qaData: QAResponse, originalQuestion: string): Question => {
  ...
  const hasAnswered = useMemo(
    () => (currentQuestion ? showResults[currentQuestion.id] : false),
  ...
  const isCorrect = useMemo(
    ...
    [hasAnswered, currentQuestion, selectedAnswer],
  );
  ...
  const showExplanation = useMemo(
    () => (currentQuestion ? showExplanations[currentQuestion.id] : false),
        ...
        <div className="space-y-4">
          ...
          <div className="pt-4 border-t border-gray-200 dark:border-gray-600">
            {!hasAnswered && selectedAnswer !== undefined ? (
              ...
              <div className="space-y-3">
                <div
                  className={`p-3 rounded-lg ${isCorrect ? "bg-green-50 dark:bg-green-900/20" : "bg-red-50 dark:bg-red-900/20"}`}
                >
          ...
          <button
            ...
            title={
              userResponses.length === 0
                ? "Answer questions to export responses"
          ...
          <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2">
            {questions.map((question, index) => {
              ...
              if (isCurrent) {
                ...
                buttonClass +=
                  "bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:border-blue-300 hover:bg-blue-50 dark:hover:bg-blue-900/20";
              }
