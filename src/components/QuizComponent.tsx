...
import { Check, X, Clock, Target, Trophy, RefreshCw, Volume2 } from 'lucide-react';
...
export const QuizComponent: React.FC<QuizComponentProps> = ({
  ...
  const questions = useMemo(() => {
    ...
    const selectedPhrases = shuffledPhrases.slice(0, Math.min(questionCount, phrases.length));
    
    return selectedPhrases.map((phrase, index) => {
      const questionTypes: Array<QuizQuestion['type']> = ['definition', 'translation', 'context', 'multipleChoice'];
      const randomType = questionTypes[Math.floor(Math.random() * questionTypes.length)];
      
      return generateQuestion(phrase, randomType, index.toString(), selectedPhrases);
    });
  ...
  const handleSubmitAnswer = (timeUp: boolean = false) => {
    ...
    const timeSpent = Math.round((questionEndTime.getTime() - questionStartTime.getTime()) / 1000);
    ...
    if (currentQuestion.type === 'multipleChoice') {
      if (selectedOption !== null && currentQuestion.options) {
        ...
        isCorrect = finalAnswer.toLowerCase().trim() === currentQuestion.correctAnswer.toLowerCase().trim();
      }
    } else {
    ...
    setTimeout(() => {
      if (currentQuestionIndex + 1 >= questions.length || (timeRemaining !== null && timeRemaining <= 0)) {
        // Quiz complete
        ...
        const results: QuizResults = {
          ...
          timeSpent: totalTime,
          accuracy: Math.round((newAnswers.filter(a => a.isCorrect).length / questions.length) * 100),
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm">
        ...
        {timeRemaining !== null && (
          <div className={`flex items-center gap-2 ${timeRemaining < 30 ? 'text-red-600' : 'text-gray-600 dark:text-gray-400'}`}>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm">
        <div className="space-y-4">
          {/* Question metadata */}
          <div className="flex items-center gap-2">
            <span className={`px-3 py-1 rounded-full text-sm ${getCategoryColor(currentQuestion.phrase.category)}`}>
              {currentQuestion.phrase.category}
            </span>
            <span className={`px-3 py-1 rounded-full text-sm ${getDifficultyColor(currentQuestion.phrase.difficulty)}`}>
              ...
              <div className="space-y-2 text-sm">
                {!answers[currentQuestionIndex]?.isCorrect && (
                  <p className="text-gray-700 dark:text-gray-300">
                    <strong>Your answer:</strong> {answers[currentQuestionIndex]?.userAnswer || 'No answer'}
                  </p>
                )}
                <p className="text-gray-700 dark:text-gray-300">
          ...
          {!showResult && (
            <button
              ...
              disabled={
                (currentQuestion.type === 'multipleChoice' && selectedOption === null) ||
                (currentQuestion.type !== 'multipleChoice' && !userAnswer.trim())
              }
      ...
      <div className="text-center text-xs text-gray-500 dark:text-gray-400">
        <p><strong>Tip:</strong> Press Enter to submit your answer</p>
      </div>
    </div>
  );
};
...
// Helper functions
function generateQuestion(phrase: SavedPhrase, type: QuizQuestion['type'], id: string, allPhrases: SavedPhrase[]): QuizQuestion {
  const baseQuestion: Omit<QuizQuestion, 'question' | 'correctAnswer' | 'options' | 'hint'> = {
    id,
    phrase,
    type
  };
  ...
  switch (type) {
    ...
    case 'context':
      return {
        ...baseQuestion,
        question: 'Complete the context: "' + phrase.context.replace(phrase.phrase, '____') + '"',
        correctAnswer: phrase.phrase,
        hint: `This is a ${phrase.partOfSpeech} in the ${phrase.category} category`
      };
      
    case 'multipleChoice':
      const otherPhrases = allPhrases.filter(p => p.id !== phrase.id && p.category === phrase.category);
      const wrongOptions = otherPhrases
        .sort(() => Math.random() - 0.5)
        .slice(0, 3)
        .map(p => p.definition);
      
      const options = [phrase.definition, ...wrongOptions].sort(() => Math.random() - 0.5);
