...
import { BookOpen, Trash2, Download, Upload, Play, Pause, RotateCcw, Star, Award, Clock, BarChart3, Settings, FileDown, FileUp, TrendingUp } from 'lucide-react';
...
import SpacedRepetitionSystem, { ReviewItem, StudyStatistics } from '@/lib/algorithms/spacedRepetition';
...
const VocabularyBuilder: React.FC<VocabularyBuilderProps> = ({
  ...
  // Start study session
  const startStudySession = useCallback((mode: 'flashcards' | 'quiz' | 'review', setId?: string) => {
  ...
  const handleFlashcardAnswer = useCallback((quality: number) => {
    ...
    const updatedPhrase: SavedPhrase = {
      ...
      studyProgress: {
        ...
        correctAnswers: currentPhrase.studyProgress.correctAnswers + (isCorrect ? 1 : 0),
        lastReviewed: new Date(),
        nextReview: reviewItem ? SpacedRepetitionSystem.calculateNextReview(reviewItem, quality).nextReview : undefined
      }
    };
    ...
    const updatedSet: VocabularySet = {
      ...currentSet,
      phrases: currentSet.phrases.map(p => p.id === currentPhrase.id ? updatedPhrase : p),
      lastModified: new Date()
    };
  ...
  const handleQuizComplete = useCallback((results: QuizResults) => {
    ...
    const sessionData: StudySession = {
      ...
      duration: Math.round((new Date().getTime() - studySession.startTime.getTime()) / (1000 * 60)),
    ...
    // Update statistics
    const updatedReviewItems = reviewItems.map(item => {
      const questionResult = results.questionsWithAnswers.find(qa => qa.question.phrase.id === item.id);
      if (questionResult) {
  ...
  const deleteVocabularySet = useCallback((setId: string) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {savedPhrases.length > 0 && !showCreateSet && viewMode.current === 'sets' && (
            <button
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold">
                {studySession.mode === 'flashcards' ? 'Flashcards' : studySession.mode === 'quiz' ? 'Quiz' : 'Review'} - {currentSet.name}
              </h3>
          ...
          {studySession.mode === 'flashcards' && (
            <FlashcardComponent
              ...
              totalCount={studySession.mode === 'review' ? (studySession.reviewItems?.length || 0) : currentSet.phrases.length}
              showNavigation={true}
            />
          )}
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-4">
                {vocabularySets.map((set) => {
                  const dueForReview = reviewItems.filter(item => 
                    set.phrases.some(phrase => phrase.id === item.id) &&
                    (!item.nextReview || item.nextReview <= new Date())
                  ).length;
                  
                  return (
                    <div key={set.id} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg p-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <button
                          onClick={() => startStudySession('flashcards', set.id)}
      ...
      {/* Empty State */}
      {viewMode.current === 'sets' && savedPhrases.length === 0 && vocabularySets.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500 mb-4">
            Save phrases from the extraction panel to start building your vocabulary.
          </p>
          <button
            onClick={importSet}
            className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
