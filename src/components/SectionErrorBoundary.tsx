...
import { AlertCircle, RefreshCw, Image, MessageSquare, BookOpen } from 'lucide-react';

interface SectionErrorBoundaryProps {
  children: ReactNode;
  sectionName: string;
  sectionType: 'image-gallery' | 'qa-panel' | 'phrases-panel' | 'descriptions' | 'search' | 'general';
  onRetry?: () => void;
  fallback?: ReactNode;
}
 ...
 */
export const SectionErrorBoundary = memo<SectionErrorBoundaryProps>(function SectionErrorBoundary({
  children,
  sectionName,
  sectionType,
  onRetry,
    ...
    switch (sectionType) {
      case 'image-gallery':
        return {
          ...
          description: 'The image gallery failed to load. This might be due to network issues or problems with the image service.',
          suggestions: [
      ...
      case 'qa-panel':
        return {
          ...
          description: 'The question and answer panel encountered an error. This could be due to API issues or problems generating questions.',
          suggestions: [
      ...
      case 'phrases-panel':
        return {
          ...
          description: 'The phrases and vocabulary panel failed to load. This might be due to issues with the phrase extraction service.',
          suggestions: [
      ...
      case 'descriptions':
        return {
          ...
          description: 'Failed to generate or load image descriptions. This could be due to AI service issues.',
          suggestions: [
      ...
      case 'search':
        return {
          ...
          title: 'Search Error',
          description: 'The image search functionality encountered an error. This might be due to network issues or search service problems.',
          suggestions: [
            'Check your internet connection',
  ...
  const customFallback = useMemo(() => fallback || (
    <div className={`
  ...
  return (
    <ErrorBoundary
      ...
      resetKeys={[sectionType, sectionName]}
    >
      {children}
    </ErrorBoundary>
  );
 ...
 */
export const ImageGalleryErrorBoundary = ({ children, onRetry }: { 
  children: ReactNode; 
  onRetry?: () => void; 
}) => (
...
export const QAPanelErrorBoundary = ({ children, onRetry }: { 
  children: ReactNode; 
  onRetry?: () => void; 
}) => (
  <SectionErrorBoundary
...
export const PhrasesPanelErrorBoundary = ({ children, onRetry }: { 
  children: ReactNode; 
  onRetry?: () => void; 
}) => (
  <SectionErrorBoundary
...
export const DescriptionsErrorBoundary = ({ children, onRetry }: { 
  children: ReactNode; 
  onRetry?: () => void; 
}) => (
  <SectionErrorBoundary
