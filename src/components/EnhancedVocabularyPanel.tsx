...
const EnhancedVocabularyPanel: React.FC<EnhancedVocabularyPanelProps> = ({
  ...
  const [extractionSettings, setExtractionSettings] = useState({
    ...
    enabledCategories: new Set<PhraseCategory>(['sustantivos', 'verbos', 'adjetivos', 'adverbios', 'frasesClaves']),
  ...
  useEffect(() => {
    if (extractionSettings.coordinateWithAlpha1 && onTabChange) {
      ...
      const coordinationData = {
        agent: 'gamma-3',
        status: 'ready',
        capabilities: ['vocabulary-extraction', 'phrase-categorization', 'csv-export'],
        preferences: extractionSettings
      };
      
      // Store coordination data in memory for Delta-4
      if (typeof window !== 'undefined') {
        window.sessionStorage.setItem('gamma3-coordination', JSON.stringify(coordinationData));
      }
    }
  ...
  const quickExtractFromDescription = useCallback(async () => {
    ...
    try {
      ...
      if (allPhrases.length <= 5) {
        const addedPhrases = await vocabularyManager.addMultiplePhrases(allPhrases, {
          autoTranslate: true,
          markAsNew: true
        });
        
        if (addedPhrases.length > 0) {
          alert(`Automatically added ${addedPhrases.length} phrases to vocabulary!`);
        }
      }
  ...
  }, [descriptionText, selectedImage, extractionSettings, vocabularyManager, handlePhrasesUpdated]);
  ...
  const categoryStats = useMemo(() => {
    ...
    return categories.map(config => ({
      ...
      percentage: panelState.vocabularyStats.totalPhrases > 0 
        ? Math.round((panelState.vocabularyStats.byCategory[config.name] || 0) / panelState.vocabularyStats.totalPhrases * 100)
        : 0
    }));
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-4 mt-2 text-sm text-gray-600 dark:text-gray-400">
            ...
            <span className="flex items-center gap-1">
              ...
              {Object.values(panelState.vocabularyStats.byCategory).filter(count => count > 0).length} categories
            </span>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex bg-gray-100 dark:bg-gray-700 rounded-lg p-1">
            <button
              onClick={() => setPanelState(prev => ({ ...prev, activeView: 'extractor' }))}
            ...
            <button
              onClick={() => setPanelState(prev => ({ ...prev, activeView: 'builder' }))}
              className={`px-3 py-1.5 text-sm rounded-md transition-colors ${
          ...
          <button
            onClick={() => setPanelState(prev => ({ ...prev, showSettings: !prev.showSettings }))}
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg p-6 space-y-4">
          ...
          <div className="space-y-3">
            <div className="flex items-center">
              ...
              />
              <label htmlFor="autoExtract" className="ml-2 text-sm text-gray-700 dark:text-gray-300">
            ...
            <div className="flex items-center">
              <input
                ...
                className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500"
              />
              <label htmlFor="coordinateAlpha1" className="ml-2 text-sm text-gray-700 dark:text-gray-300">
                Coordinate with Alpha-1 description tabs
              </label>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {categoryStats.map(category => (
          ...
          >
            <div className={`w-8 h-8 mx-auto mb-2 rounded-full flex items-center justify-center text-xs font-bold ${category.color}`}>
              {category.count}
            </div>
