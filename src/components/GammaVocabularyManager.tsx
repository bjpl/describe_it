...
const GammaVocabularyManager: React.FC<GammaVocabularyManagerProps> = ({
  ...
  useEffect(() => {
    if (settings.coordinateWithAlpha1 && typeof window !== 'undefined') {
      ...
      window.sessionStorage.setItem('gamma3-manager-status', JSON.stringify(coordinationData));
  ...
  const handlePhrasesUpdated = useCallback((phrases: CategorizedPhrase[]) => {
    ...
    if (settings.logToDelta4 && onLogEvent) {
      onLogEvent({
        ...
        data: {
          ...
          categories: phrases.reduce((acc, phrase) => {
  ...
  const exportVocabularyCSV = useCallback(async (setId?: string) => {
      ...
      if (settings.logToDelta4 && onLogEvent) {
        onLogEvent({
          ...
          data: {
            ...
            totalPhrases: setId 
              ? vocabularySets.find(set => set.id === setId)?.phrases.length || 0
  ...
  }, [vocabularyManager, settings.enableTranslation, settings.logToDelta4, onLogEvent, vocabularySets, state.vocabularyStats.totalPhrases]);

  // Create new vocabulary set from selected phrases
  const createVocabularySet = useCallback(async (name: string, description?: string) => {
  ...
  const deleteVocabularySet = useCallback(async (setId: string) => {
  ...
  const filteredPhrases = useMemo(() => {
    ...
    return allPhrases.filter(phrase => {
      const matchesSearch = !state.searchTerm || 
        ...
        (phrase.translation && phrase.translation.toLowerCase().includes(state.searchTerm.toLowerCase()));
      
      const matchesCategory = state.selectedCategory === 'all' || phrase.category === state.selectedCategory;
      const matchesDifficulty = state.selectedDifficulty === 'all' || phrase.difficulty === state.selectedDifficulty;
      
      return matchesSearch && matchesCategory && matchesDifficulty;
    });
  }, [vocabularySets, state.searchTerm, state.selectedCategory, state.selectedDifficulty]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2">
            ...
            Vocabulary Manager
            <span className="text-sm font-normal text-gray-500">Agent Gamma-3</span>
          </h2>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex bg-gray-100 dark:bg-gray-700 rounded-lg p-1">
            {(['extractor', 'builder', 'stats', 'sets'] as const).map((view) => (
              <button
                key={view}
          ...
          <motion.button
            ...
            disabled={state.vocabularyStats.totalPhrases === 0 || state.isExporting}
        ...
        {state.showSettings && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="space-y-3">
                ...
                <div className="flex items-center">
                  <input
                    ...
                    className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500"
                  />
                  <label htmlFor="sortIgnoreArticles" className="ml-2 text-sm text-gray-700 dark:text-gray-300">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center">
                  <input
                    ...
                    className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500"
                  />
                  <label htmlFor="enableTranslation" className="ml-2 text-sm text-gray-700 dark:text-gray-300">
                    Enable translations
                  </label>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setSettings(prev => ({
                      ...prev,
                      displayDensity: e.target.value as 'compact' | 'comfortable' | 'spacious'
                    }))}
              ...
              <div className="space-y-3">
                <h4 className="font-medium text-gray-900 dark:text-gray-100">Coordination</h4>
                <div className="flex items-center">
                  <input
                    type="checkbox"
                    ...
                    className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500"
                  />
                  <label htmlFor="coordinateWithAlpha1" className="ml-2 text-sm text-gray-700 dark:text-gray-300">
                ...
                <div className="flex items-center">
                  <input
                    ...
                    className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500"
                  />
                  <label htmlFor="logToDelta4" className="ml-2 text-sm text-gray-700 dark:text-gray-300">
                    Log to Delta-4
                  </label>
                </div>
              </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {categoryConfigs.map(config => {
          ...
          const percentage = state.vocabularyStats.totalPhrases > 0 
          ...
          return (
            <motion.div
              ...
              onClick={() => setState(prev => ({ 
                ...prev, 
                selectedCategory: prev.selectedCategory === config.name ? 'all' : config.name,
                activeView: 'builder'
              }))}
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
              <div className={`w-8 h-8 mx-auto mb-2 rounded-full flex items-center justify-center text-xs font-bold ${config.color}`}>
                {count}
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <select
                ...
                onChange={(e) => setState(prev => ({ 
                  ...prev, 
                  selectedDifficulty: e.target.value as 'all' | 'beginner' | 'intermediate' | 'advanced' 
                }))}
                className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700"
              >
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="text-sm text-gray-600 dark:text-gray-400">
                  {selectedPhrases.size} of {filteredPhrases.length} phrases selected
                </span>
                    ...
                    >
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center gap-2 ml-4">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${getCategoryColor(phrase.category)}`}>
                            {phrase.category}
                          </span>
        ...
        {state.activeView === 'stats' && (
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg p-6">
              ...
              <div className="space-y-3">
                {(['beginner', 'intermediate', 'advanced'] as const).map(level => {
                  const count = state.vocabularyStats.difficultyDistribution[level] || 0;
                  const percentage = state.vocabularyStats.totalPhrases > 0 
            ...
            {state.vocabularyStats.recentlyAdded.length > 0 && (
              <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg p-6">
                <h3 className="text-lg font-semibold mb-4">Recently Added Phrases</h3>
                <div className="space-y-2">
                  {state.vocabularyStats.recentlyAdded.slice(0, 10).map(phrase => (
                    <div key={phrase.id} className="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
                      <div>
                        <span className="font-medium">{phrase.phrase}</span>
                        <span className="text-gray-600 dark:text-gray-400 ml-2">- {phrase.definition}</span>
                      </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <motion.button
                onClick={() => {
                  ...
                  if (setName) createVocabularySet(setName, 'Custom vocabulary set');
                }}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
                          ...
                          <span>Modified {set.lastModified.toLocaleDateString()}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 ml-4">
                        ...
                        <motion.button
                          onClick={() => setState(prev => ({ 
                            ...prev, 
                            editingSetId: prev.editingSetId === set.id ? null : set.id 
                          }))}
              ...
              {vocabularySets.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600 dark:text-gray-400 mb-6">
                    Create vocabulary sets to organize your Spanish phrases and track your learning progress.
                  </p>
                  <motion.button
                    onClick={() => {
                      const setName = prompt('Enter vocabulary set name:');
