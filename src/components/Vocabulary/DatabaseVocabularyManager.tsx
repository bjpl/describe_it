...
export const DatabaseVocabularyManager: React.FC<VocabularyManagerProps> = ({
  ...
  const handleBulkAction = useCallback((action: 'study' | 'export' | 'delete') => {
    if (selectedItems.length === 0) return;
    
    switch (action) {
      case 'study':
        // Implement study session with selected items
        console.log('Starting study session with', selectedItems.length, 'items');
        break;
      case 'export':
    ...
    <div className={`space-y-6 ${className}`}>
        ...
        <div className="flex items-center gap-2 text-sm">
          {isConnected ? (
            <>
              ...
              <span className="text-green-600 dark:text-green-400">Connected to Database</span>
              <Database className="w-4 h-4 text-green-500" />
            </>
          ) : (
            <>
              <WifiOff className="w-4 h-4 text-orange-500" />
              <span className="text-orange-600 dark:text-orange-400">Using Sample Data</span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-lg font-semibold">{Object.keys(stats.byCategory).length}</div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-lg font-semibold">{stats.averageDifficulty}</div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-lg font-semibold">{stats.averageFrequency}</div>
      ...
      <div className="space-y-4">
        {loading && !items.length ? (
          ...
          items.map((item) => (
            <div key={item.id} className={`bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 transition-all duration-200 ${
              selectedItems.includes(item.id) ? 'ring-2 ring-blue-500 ring-opacity-50' : ''
            }`}>
              <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className="font-medium text-lg">{item.spanish_text}</h3>
                        {item.frequency_score && item.frequency_score > 80 && (
                          <Star className="w-4 h-4 text-yellow-500 fill-current" />
                        )}
                      </div>
                      <p className="text-gray-600 dark:text-gray-400 mb-2">{item.english_translation}</p>
                      ...
                      <div className="flex flex-wrap gap-2 mt-3">
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${getDifficultyColor(item.difficulty_level)}`}>
                ...
                {allowEdit && (
                  <div className="flex flex-col gap-2">
                    <button className="p-2 border border-gray-300 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors" title="Edit">
                      <Edit3 className="w-4 h-4" />
                    </button>
                    
                    <button className="p-2 border border-gray-300 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors" title="Pronunciation">
                      <Volume2 className="w-4 h-4" />
                    </button>

                    <button className="p-2 border border-gray-300 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors" title="Add to Study List">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-8 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-600 dark:text-gray-300 mb-2">No vocabulary found</h3>
