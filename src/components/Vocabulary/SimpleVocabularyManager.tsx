...
export const SimpleVocabularyManager: React.FC<VocabularyManagerProps> = ({
  ...
  const filteredPhrases = useMemo(() => {
    return phrases.filter(phrase => {
      const matchesSearch = searchQuery === '' || 
        ...
        phrase.english_translation.toLowerCase().includes(searchQuery.toLowerCase());
      
      const matchesCategory = selectedCategory === 'all' || phrase.category === selectedCategory;
      const matchesDifficulty = selectedDifficulty === 'all' || phrase.difficulty_level === selectedDifficulty;
  ...
  }, [phrases, searchQuery, selectedCategory, selectedDifficulty, selectedFilter]);
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400">Total Phrases</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400">Mastery Rate</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-4">
        {filteredPhrases && filteredPhrases.length > 0 ? (
          filteredPhrases.map((phrase) => (
            <div key={phrase.id} className={`bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 transition-all duration-200 ${
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex-1 space-y-2">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-gray-600 dark:text-gray-400 mb-2">{phrase.english_translation}</p>
                      
                      {/* Context sentences */}
                      {phrase.context_sentence_spanish && (
                        <div className="text-sm bg-gray-50 dark:bg-gray-700 p-2 rounded">
                          <p className="italic text-gray-700 dark:text-gray-300">{phrase.context_sentence_spanish}</p>
                          {phrase.context_sentence_english && (
                            <p className="text-gray-500 dark:text-gray-400 mt-1">{phrase.context_sentence_english}</p>
                          )}
                        </div>
                      )}
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${getDifficultyColor(phrase.difficulty_level)}`}>
                          {phrase.difficulty_level}
                        </span>
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${getCategoryColor(phrase.category)}`}>
                        ...
                        <div className="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400 mt-2">
                          ...
                          <span>
                            Accuracy: {phrase.study_count! > 0 
                              ? Math.round(((phrase.correct_count || 0) / phrase.study_count!) * 100)
                              : 0}%
                          </span>
                ...
                {allowEdit && (
                  <div className="flex flex-col gap-2">
                    <button className={`px-3 py-1 text-sm rounded-md border transition-colors ${
                      phrase.is_user_selected 
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-8 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-600 dark:text-gray-300 mb-2">No phrases found</h3>
