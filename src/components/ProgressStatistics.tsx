...
export const ProgressStatistics: React.FC<ProgressStatisticsProps> = ({
  ...
  const studyProgressData: ChartData[] = React.useMemo(() => {
    ...
    return last30Days.map((date) => {
      const sessionsForDate = studyHistory.filter(
        (session) => session.date === date,
      );
      const totalStudied = sessionsForDate.reduce(
        (sum, session) => sum + session.itemsStudied,
        0,
      );
      const totalCorrect = sessionsForDate.reduce(
  ...
  const masteryData: ChartData[] = React.useMemo(() => {
    const totalPhrases = vocabularySets.reduce(
      (sum, set) => sum + set.phrases.length,
      0,
    );
    ...
    const learningPhrases = vocabularySets.reduce(
      (sum, set) =>
        sum +
        set.phrases.filter(
          (phrase: any) =>
  ...
  const thisWeekSessions = React.useMemo(() => {
    ...
    return studyHistory.filter(
      (session) => new Date(session.date) >= oneWeekAgo,
    );
  }, [studyHistory]);

  const weeklyItemsStudied = thisWeekSessions.reduce(
    (sum, session) => sum + session.itemsStudied,
    0,
  );
  ...
  const weeklyProgress = Math.min(
    100,
    Math.round((weeklyItemsStudied / weeklyGoal) * 100),
  );
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white p-4 rounded-lg">
          ...
          <p className="text-blue-100 text-xs mt-1">
            ~{statistics.estimatedTime} minutes
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          ...
          <div>
            <div className="text-2xl font-bold text-purple-600">
              {Math.round(
                (statistics.correctReviews /
                  Math.max(statistics.totalReviews, 1)) *
                  100,
              )}
              %
            </div>
