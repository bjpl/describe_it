...
export function usePhraseExtraction(imageId: string) {
  ...
  const createPhraseError = (error: unknown, response?: Response): PhraseError => {
    if (error instanceof Error) {
      ...
      if (error.name === 'AbortError') {
        return {
          message: 'Request timed out. Phrase extraction is taking too long. Please try again.',
          type: 'timeout',
          retryable: true
        };
      }
    ...
    if (response && !response.ok) {
      ...
      switch (statusCode) {
        case 400:
          message = 'Invalid request parameters. Please check the image URL and settings.';
          type = 'validation';
          break;
        case 401:
        ...
        case 403:
          ...
          break;
        case 429:
          message = 'Too many requests. Please wait before extracting more phrases.';
          type = 'server';
          retryable = true;
          break;
        case 500:
  ...
  const makePhraseRequest = async (request: PhraseExtractionRequest): Promise<ExtractedPhrase[]> => {
    ...
    try {
      ...
      if (!Array.isArray(data)) {
        throw new Error('Invalid response format from phrase extraction service');
      }
      
      // Validate each phrase object
      const validPhrases = data.filter(phrase => 
        phrase && 
  ...
  const retryPhraseRequest = async (request: PhraseExtractionRequest): Promise<ExtractedPhrase[]> => {
  ...
  const extractPhrases = useCallback(async (request: PhraseExtractionRequest): Promise<ExtractedPhrase[]> => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      if (processedRequest.maxPhrases < 1 || processedRequest.maxPhrases > 20) {
