...
export function useQuestionAnswer(imageId: string) {
  ...
  const createQAError = (error: unknown, response?: Response): QAError => {
    if (error instanceof Error) {
      ...
      if (error.name === 'AbortError') {
        return {
          message: 'Request timed out. AI processing is taking too long. Please try a simpler question.',
          type: 'timeout',
          retryable: true
        };
      }
    ...
    if (response && !response.ok) {
      ...
      switch (statusCode) {
        case 400:
          message = 'Invalid question or image. Please check your input and try again.';
          type = 'validation';
          break;
        case 401:
        ...
        case 403:
          ...
          break;
        case 429:
          message = 'Too many questions asked. Please wait a moment before asking another question.';
          type = 'server';
          retryable = true;
          break;
        case 500:
  ...
  const makeQARequest = async (request: QARequest): Promise<QuestionAnswerPair> => {
  ...
  const retryQARequest = async (request: QARequest): Promise<QuestionAnswerPair> => {
