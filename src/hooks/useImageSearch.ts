...
export function useImageSearch() {
  ...
  const createSearchError = (error: unknown, response?: Response): SearchError => {
    ...
    if (response && !response.ok) {
      ...
      switch (statusCode) {
        case 400:
          message = 'Invalid search parameters. Please check your search query.';
          type = 'validation';
          break;
        case 401:
  ...
  const makeSearchRequest = async (query: string, page: number = 1): Promise<any> => {
  ...
  const retryRequest = async (query: string, page: number = 1): Promise<any> => {
  ...
  const loadMoreImages = useCallback(async () => {
    if (searchParams.page >= totalPages || loading.isLoading || !searchParams.query) return;

    const nextPage = searchParams.page + 1;
    await searchImages(searchParams.query, nextPage);
  }, [searchParams.query, searchParams.page, totalPages, loading.isLoading, searchImages]);
