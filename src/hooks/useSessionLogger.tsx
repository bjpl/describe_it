...
export function useSessionLogger(
  options: UseSessionLoggerOptions = {},
): UseSessionLoggerReturn {
  const { autoStart = true, persistToStorage = true, settings = {} } = options;
  ...
  useEffect(() => {
    ...
    if (!sessionLoggerRef.current && autoStart) {
      ...
      reportGeneratorRef.current = new SessionReportGenerator(
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        sessionLoggerRef.current?.logInteraction("session_ended", {
          duration:
            Date.now() -
            sessionLoggerRef.current.getSessionMetadata().startTime,
        });
      } else {
  ...
  const logInteraction = useCallback(
    (type: InteractionType, data: InteractionData = {}) => {
  ...
  const logImageSelection = useCallback(
    (imageId: string, imageUrl: string, selectionTime: number) => {
      sessionLoggerRef.current?.logImageSelection(
        imageId,
        imageUrl,
        selectionTime,
      );
  ...
  const logDescriptionGeneration = useCallback(
    (
      ...
      sessionLoggerRef.current?.logDescriptionGeneration(
        style,
        language,
        wordCount,
        generationTime,
  ...
  const logQAGeneration = useCallback(
    (
      ...
      sessionLoggerRef.current?.logQAGeneration(
        question,
        answer,
        difficulty,
        category,
  ...
  const logVocabularySelection = useCallback(
  ...
  const logPhraseExtraction = useCallback(
    (phrases: string[], categories: Record<string, string[]>) => {
  ...
  const logSettingsChange = useCallback(
    (settingName: string, oldValue: unknown, newValue: unknown) => {
      sessionLoggerRef.current?.logSettingsChange(
        settingName,
        oldValue,
        newValue,
      );
  ...
  const exportSession = useCallback(
    (format: "json" | "text" | "csv" = "json"): string => {
      if (!sessionLoggerRef.current) {
        throw new Error("Session logger not initialized");
      }
  ...
  const saveSession = useCallback(async () => {
    ...
    await persistenceRef.current.save(
      sessionLoggerRef.current.getSessionId(),
      sessionData,
    );
    await persistenceRef.current.saveSummary(
      sessionLoggerRef.current.getSessionId(),
      summary,
    );
  }, []);
  ...
  const updateSettings = useCallback(
    (newSettings: Partial<SessionLoggerSettings>) => {
      if (!sessionLoggerRef.current) return;
  ...
  const startTime =
    sessionLoggerRef.current?.getSessionMetadata()?.startTime || Date.now();
  const interactionCount =
    sessionLoggerRef.current?.getInteractions()?.length || 0;
  const currentSettings =
    sessionLoggerRef.current?.getSettings() || ({} as SessionLoggerSettings);
...
export function useSearchLogging() {
  ...
  return useCallback(
    async (searchFn: (query: string) => Promise<any>, query: string) => {
...
export function useImageInteractionLogging() {
  ...
  const logImageView = useCallback(
  ...
  const logImageClick = useCallback(
    (imageId: string, imageUrl: string, selectionTime: number) => {
...
export function useContentGenerationLogging() {
  ...
  const logDescriptionGenerated = useCallback(
    (style: string, language: string, text: string, generationTime: number) => {
  ...
  const logGenerationError = useCallback(
