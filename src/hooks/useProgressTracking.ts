...
function calculateTrend(history: number[]): 'improving' | 'stable' | 'declining' {
...
export function useProgressStats() {
  ...
  useEffect(() => {
    ...
    const todayAccuracy = dailyProgress.total > 0 
      ? (dailyProgress.correct / dailyProgress.total) * 100 
      : 0;

    // Calculate completion rate from accuracy history
    const completionRate = sessionData.accuracy_history.length > 0
      ? sessionData.accuracy_history.reduce((a: number, b: number) => a + b, 0) / sessionData.accuracy_history.length
      : 0;
    ...
    const achievements = calculateAchievements({
      ...
      streak_current: getStorageItem(STORAGE_KEYS.STREAK_INFO, { current: 0 }).current,
    ...
    const pointsToNextLevel = Math.ceil((sessionData.total_points / 100) + 1) * 100;
  ...
  useEffect(() => {
    ...
    Object.entries(analytics.skill_scores).forEach(([skill, scores]) => {
      ...
      if (recentScores.length > 0) {
        skillBreakdown[skill] = recentScores.reduce((a, b) => a + b, 0) / recentScores.length;
      } else {
    ...
    const recentSessions = analytics.recent_sessions.filter((session: any) => 
      new Date(session.date).getTime() > weekAgo
    );
    const recentPhrases = analytics.phrases_learned.filter((phrase: any) =>
      new Date(phrase.date).getTime() > weekAgo
    );
    ...
    const focusAreas = sortedSkills.map(([skill, score]) => {
      if (skill === 'vocabulary' && score < 70) return 'Expand vocabulary practice';
      if (skill === 'grammar' && score < 70) return 'Grammar patterns review';
      if (skill === 'comprehension' && score < 70) return 'Reading comprehension exercises';
    ...
    const learningAnalytics: LearningAnalytics = {
      ...
      recent_activity: {
        ...
        descriptions_completed: recentSessions.reduce((sum: number, s: any) => sum + (s.descriptions || 0), 0),
...
export function useProgressSummary() {
  ...
  useEffect(() => {
    ...
    const accuracyRate = sessionData.total_attempts > 0
...
export function updateProgress(type: 'session' | 'description' | 'phrase' | 'quiz', result?: any) {
  const sessionData = getStorageItem(STORAGE_KEYS.SESSION_DATA, {
    total_sessions: 0,
    total_points: 0,
    total_descriptions: 0,
  ...
  switch (type) {
    ...
    case 'phrase':
      ...
      // Update analytics
      const analytics = getStorageItem(STORAGE_KEYS.ANALYTICS, {
        skill_scores: { vocabulary: [], grammar: [], comprehension: [], speaking: [] },
        recent_sessions: [],
        phrases_learned: []
      });
