...
const envSchema = z.object({
  ...
  PORT: createPortValidator().default(3000).describe("Server port number"),
  ...
  // -------------------------------------------------------------------------
  NEXT_PUBLIC_SUPABASE_URL: createUrlValidator().describe(
  ...
  // -------------------------------------------------------------------------
  KV_REST_API_URL: createUrlValidator().describe("Vercel KV database URL"),
  ...
  // -------------------------------------------------------------------------
  SENTRY_DSN: createUrlValidator().describe("Sentry DSN for error tracking"),
  ...
  // -------------------------------------------------------------------------
  VERCEL_TOKEN: z.string().optional().describe("Vercel deployment token"),
  ...
  // -------------------------------------------------------------------------
  SNYK_TOKEN: z.string().optional().describe("Snyk security scanning token"),
  ...
  // -------------------------------------------------------------------------
  SLACK_WEBHOOK_URL: createUrlValidator().describe(
    "Slack webhook URL for notifications",
  ),

  DISCORD_WEBHOOK_URL: createUrlValidator().describe(
  ...
  // -------------------------------------------------------------------------
  UNSPLASH_RATE_LIMIT_PER_HOUR: createNumberValidator(1, 10000, 1000).describe(
    "Unsplash API rate limit per hour",
  ),

  OPENAI_RATE_LIMIT_PER_MINUTE: createNumberValidator(1, 10000, 60).describe(
    "OpenAI API rate limit per minute",
  ),

  API_RATE_LIMIT_WINDOW: createNumberValidator(1, 3600, 900).describe(
  ...
  // -------------------------------------------------------------------------
  DEFAULT_CACHE_TTL: createNumberValidator(1, 86400, 3600).describe(
    "Default cache TTL in seconds",
  ),

  MAX_CACHE_SIZE: createNumberValidator(1, 10000, 1000).describe(
    "Maximum cache size (number of items)",
  ),

  REDIS_URL: createUrlValidator().describe("Redis connection URL for caching"),

  // -------------------------------------------------------------------------
  // Feature Flags & Toggles
  // -------------------------------------------------------------------------
  ENABLE_DEMO_MODE: createBooleanValidator(false).describe(
    "Force enable demo mode",
  ),

  DEMO_MODE_AUTO: createBooleanValidator(true).describe(
    "Auto-enable demo mode when API keys missing",
  ),

  ENABLE_FEATURE_FLAGS: createBooleanValidator(false).describe(
    "Enable feature flags system",
  ),

  ENABLE_ANALYTICS: createBooleanValidator(true).describe(
    "Enable analytics tracking",
  ),

  ENABLE_DEBUG_MODE: createBooleanValidator(false).describe(
    "Enable debug logging and features",
  ),

  MAINTENANCE_MODE: createBooleanValidator(false).describe(
  ...
  // -------------------------------------------------------------------------
  NEXT_TELEMETRY_DISABLED: z.coerce
    .number()
    .default(1)
    .describe("Disable Next.js telemetry"),

  ANALYZE: createBooleanValidator(false).describe("Enable bundle analyzer"),

  BUILD_ID: z.string().optional().describe("Custom build identifier"),
  ...
  // -------------------------------------------------------------------------
  CI: createBooleanValidator(false).describe("Running in CI environment"),
  ...
  E2E_BASE_URL: z.string().optional().describe("Base URL for E2E tests"),
});
    ...
    if (error instanceof z.ZodError) {
      return {
        ...
        errors: error.issues.map((issue) => ({
          ...
          required: !issue.path.some((p) =>
            envSchema.shape[p as keyof typeof envSchema.shape]?.isOptional(),
          ),
        })),
      };
    }
...
function generateWarnings(env: Environment): string[] {
  ...
  if (env.NODE_ENV === "development") {
    if (
      ...
      !env.NEXT_PUBLIC_APP_URL.includes(":3000")
    ) {
  ...
  if (!env.NEXT_PUBLIC_UNSPLASH_ACCESS_KEY && !env.OPENAI_API_KEY) {
    warnings.push(
      "No external API keys configured - running in full demo mode",
    );
  }
  ...
  if (env.JWT_SECRET && env.JWT_SECRET.length < 64) {
    warnings.push(
      "JWT secret should be at least 64 characters for better security",
    );
  }
...
export function getServiceStatuses(): ServiceStatus[] {
  ...
  return [
    ...
    // External APIs
    {
      ...
      required: false,
      reason: env.NEXT_PUBLIC_UNSPLASH_ACCESS_KEY
        ? undefined
        : "Using demo images",
    },
...
export function isDemoMode(): boolean {
  ...
  return (
    ...
    (env.DEMO_MODE_AUTO &&
      (!env.NEXT_PUBLIC_UNSPLASH_ACCESS_KEY || !env.OPENAI_API_KEY))
  );
}
