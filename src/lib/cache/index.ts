...
export function getCacheTTL(contentType: 'images' | 'descriptions' | 'qa' | 'phrases' | 'default'): number {
  switch (contentType) {
    case 'images':
...
export async function getAllCacheMetrics() {
  ...
  const { tieredCache, imageCache, descriptionCache, qaCache, phrasesCache } = await import('./tiered-cache');
  
  const [memoryStats, tieredMetrics, imageMetrics, descMetrics, qaMetrics, phraseMetrics] = await Promise.all([
  ...
  const allMetrics = [tieredMetrics, imageMetrics, descMetrics, qaMetrics, phraseMetrics];
  const totalHitRate = allMetrics.reduce((sum, m) => sum + (m.hitRate || 0), 0) / allMetrics.length;
