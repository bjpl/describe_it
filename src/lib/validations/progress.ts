...
export const createProgressSchema = z.object({
  progress_type: z.enum(['daily', 'weekly', 'monthly', 'skill', 'vocabulary', 'grammar', 'achievement', 'milestone', 'session_summary', 'audit_log']),
  progress_date: z.string().date().optional(),
  skill_category: z.enum(['reading_comprehension', 'vocabulary_recognition', 'grammar_understanding', 'translation_accuracy', 'cultural_awareness', 'listening_comprehension']).optional(),
  current_level: z.enum(['beginner', 'intermediate', 'advanced', 'expert']).optional(),
...
export const progressFiltersSchema = z.object({
  progress_type: z.enum(['daily', 'weekly', 'monthly', 'skill', 'vocabulary', 'grammar', 'achievement', 'milestone', 'session_summary', 'audit_log']).optional(),
  skill_category: z.enum(['reading_comprehension', 'vocabulary_recognition', 'grammar_understanding', 'translation_accuracy', 'cultural_awareness', 'listening_comprehension']).optional(),
  date_from: z.string().date().optional(),
  date_to: z.string().date().optional(),
  limit: z.number().min(1).max(100).default(30),
  offset: z.number().min(0).default(0),
  sort_by: z.enum(['progress_date', 'points_earned', 'accuracy_percentage', 'improvement_rate']).default('progress_date'),
...
export const updateGoalsSchema = z.object({
  ...
  custom_goals: z.record(z.object({
...
export const achievementSchema = z.object({
  ...
  category: z.enum(['streak', 'accuracy', 'vocabulary', 'sessions', 'time', 'consistency', 'milestone']),
...
export const performanceAnalyticsSchema = z.object({
  ...
  metrics: z.array(z.enum(['accuracy', 'consistency', 'speed', 'retention', 'difficulty_progression'])).default(['accuracy', 'consistency']),
...
export type PerformanceAnalyticsRequest = z.infer<typeof performanceAnalyticsSchema>;