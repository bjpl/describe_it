...
class LocalStorageAdapter {
  ...
  async saveItem(table: string, item: any): Promise<{ data: any; error: any }> {
    try {
      const id = item.id || `${table}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      const itemWithId = { ...item, id, created_at: new Date().toISOString(), updated_at: new Date().toISOString() };
      ...
      return this.mockResponse(null, { message: `Failed to save ${table}`, error });
    }
  }
  ...
  async getItem(table: string, id: string): Promise<{ data: any; error: any }> {
    try {
      ...
      if (!item) {
        return this.mockResponse(null, { message: 'Item not found', code: 'PGRST116' });
      }
      return this.mockResponse(JSON.parse(item));
    } catch (error) {
      return this.mockResponse(null, { message: `Failed to get ${table}`, error });
    }
  }

  async getItems(table: string, filters: any = {}): Promise<{ data: any[]; error: any }> {
    try {
      const indexKey = this.getKey(`${table}_index`);
      const index = JSON.parse(localStorage.getItem(indexKey) || '[]');
      
      const items = index.map((id: string) => {
        const item = localStorage.getItem(this.getKey(table, id));
      ...
      Object.keys(filters).forEach(key => {
        if (filters[key] !== undefined) {
          filteredItems = filteredItems.filter(item => item[key] === filters[key]);
        }
      });
      
      return this.mockResponse(filteredItems);
    } catch (error) {
      return this.mockResponse([], { message: `Failed to get ${table}s`, error });
    }
  }

  async updateItem(table: string, id: string, updates: any): Promise<{ data: any; error: any }> {
    try {
      ...
      return this.mockResponse(null, { message: `Failed to update ${table}`, error });
    }
  }
  ...
  async deleteItem(table: string, id: string): Promise<{ error: any }> {
    try {
      ...
      const indexKey = this.getKey(`${table}_index`);
      const existingIndex = JSON.parse(localStorage.getItem(indexKey) || '[]');
      const updatedIndex = existingIndex.filter((itemId: string) => itemId !== id);
  ...
  async getStats(): Promise<{ data: { totalImages: number; totalDescriptions: number }; error: any }> {
    try {
      const imagesIndex = JSON.parse(localStorage.getItem(this.getKey('images_index')) || '[]');
      const descriptionsIndex = JSON.parse(localStorage.getItem(this.getKey('descriptions_index')) || '[]');
...
class SupabaseService {
  ...
  constructor() {
    ...
    if (!this.isDemo && (!this.url || !this.anonKey || 
        this.url === 'https://placeholder.supabase.co' ||
        this.anonKey === 'placeholder_anon_key')) {
      console.warn('Supabase credentials not configured. Running in demo mode with localStorage.');
      this.isDemo = true;
    }
  ...
  async saveImage(imageData: Database['public']['Tables']['images']['Insert']) {
    try {
      if (this.isDemo) {
        const { data, error } = await this.localStorage.saveItem('images', imageData);
        if (error) {
