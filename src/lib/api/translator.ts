...
class TranslatorService {
   ...
   */
  private generateCacheKey(text: string, fromLanguage: string, toLanguage: string, context?: string): string {
    const contextSuffix = context ? `:${Buffer.from(context).toString('base64').slice(0, 8)}` : '';
    const textHash = Buffer.from(text.toLowerCase().trim()).toString('base64').slice(0, 16);
   ...
   */
  private getFallbackTranslation(text: string, fromLanguage: string, toLanguage: string): string | null {
  ...
  async translateText(request: TranslationRequest): Promise<TranslationResult> {
    ...
    const cacheKey = this.generateCacheKey(text, fromLanguage, toLanguage, context);
    ...
    try {
      ...
      try {
        ...
        console.warn('OpenAI translation failed, trying fallback:', openAIError);
        
        // Try fallback translation
        const fallback = this.getFallbackTranslation(text, fromLanguage, toLanguage);
  ...
  isLanguagePairSupported(fromLanguage: string, toLanguage: string): boolean {
    ...
    return supportedCodes.includes(fromLanguage) && supportedCodes.includes(toLanguage);
  }
  ...
  async getTranslationStats(): Promise<{
    ...
    try {
      ...
      return {
        ...
        fallbackTranslationsAvailable: Object.keys(FALLBACK_TRANSLATIONS['es-en']).length
      };
    } catch {
      return {
        totalTranslations: 0,
