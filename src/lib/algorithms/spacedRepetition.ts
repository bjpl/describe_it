...
export class SpacedRepetitionSystem {
  ...
  static calculateNextReview(item: ReviewItem, quality: number): ReviewItem {
    ...
    easiness = easiness + (0.1 - (5 - clampedQuality) * (0.08 + (5 - clampedQuality) * 0.02));
   ...
   */
  static getItemsDueForReview(items: ReviewItem[], maxItems: number = 20): ReviewItem[] {
    const now = new Date();
   ...
   */
  static createReviewItem(id: string, phrase: string, definition: string): ReviewItem {
    return {
      id,
      phrase,
      definition,
  ...
  static calculateStatistics(items: ReviewItem[]): StudyStatistics {
    ...
    const totalReviews = reviewedItems.reduce((sum, item) => sum + (item.repetition || 0), 0);
    ...
    const averageQuality = reviewedItems.length > 0 
      ? reviewedItems.reduce((sum, item) => sum + (item.quality || 0), 0) / reviewedItems.length
      : 0;
    
    // Calculate study streak (consecutive days with reviews)
    let studyStreak = 0;
    const sortedReviews = reviewedItems
    ...
    for (const item of sortedReviews) {
      ...
      const daysDiff = Math.floor((currentDate.getTime() - item.lastReviewed.getTime()) / (1000 * 60 * 60 * 24));
      if (daysDiff <= 1) {
        studyStreak = Math.max(studyStreak, daysDiff === 0 ? 1 : studyStreak + 1);
    ...
    const masteredItems = items.filter(item => 
      item.repetition >= 3 && item.difficulty > 2.0
    ).length;
   ...
   */
  static responseToQuality(correct: boolean, confidence: 'low' | 'medium' | 'high' = 'medium'): number {
    if (!correct) {
   ...
   */
  static getOptimalBatchSize(totalDue: number, availableTime: number = 30): number {
   ...
   */
  static scheduleReviews(items: ReviewItem[], daysAhead: number = 7): Map<string, ReviewItem[]> {
