...
export const baseRequestSchema = z.object({
  ...
  // Session validation
  sessionId: z.string().min(10, 'Session ID too short').max(256, 'Session ID too long').optional(),
});
 ...
 */
export const authSignupSchema = baseRequestSchema
  .extend({
    email: z
      .string()
    ...
    password: z
      ...
      .max(
        128,
        'Password too long (max 128 characters) | Contraseña demasiado larga (máximo 128 caracteres)'
      )
      .refine(
...
export const authUpdatePasswordSchema = baseRequestSchema
  .extend({
...
export const metricsQuerySchema = baseRequestSchema
  .extend({
    metric: z
      .string()
...
export const auditLogSchema = baseRequestSchema.extend({
  ...
  resourceId: z.string().min(1, 'Resource ID is required').max(128, 'Resource ID too long'),
...
export const descriptionGenerateSchema = baseRequestSchema.extend({
  ...
  customPrompt: z
    .string()
    .max(500, 'Custom prompt too long')
    .refine(text => {
      // Basic XSS protection - reject common script patterns
      const dangerousPatterns = [
        /<script/i,
...
export const phraseExtractionSchema = baseRequestSchema.extend({
  ...
  language: languageCodeSchema.default('es'),
  categories: z
    .array(z.enum(['objetos', 'acciones', 'lugares', 'colores', 'emociones', 'conceptos']))
...
export const exportRequestSchema = baseRequestSchema.extend({
  userId: z.string().min(1, 'User ID required for export').max(128, 'User ID too long'),
...
export const vocabularySaveSchema = baseRequestSchema.extend({
  userId: z.string().min(1, 'User ID required').max(128, 'User ID too long'),
  phrase: z
    .string()
    .max(200, 'Phrase too long')
    .refine(text => {
      // Basic XSS protection - reject common script patterns
      const dangerousPatterns = [
        /<script/i,
...
export const settingsSaveSchema = baseRequestSchema.extend({
  ...
  settings: z
    ...
    .refine(settings => Object.keys(settings).length > 0, 'At least one setting must be provided'),
});
...
export const apiKeySaveSchema = baseRequestSchema.extend({
  ...
  name: z.string().min(1, 'API key name is required').max(100, 'API key name too long').optional(),
});
...
export const testKeyVerificationSchema = baseRequestSchema.extend({
  provider: z.enum(['openai', 'anthropic', 'google']),
  apiKey: z.string().min(20, 'API key too short').max(500, 'API key too long'),
...
export const adminAnalyticsSchema = baseRequestSchema.extend({
  ...
  metrics: z
    .array(z.enum(['requests', 'errors', 'response_time', 'users', 'conversions', 'revenue']))
...
export const searchQuerySchema = z.object({
  query: z
    ...
    .refine(query => {
      ...
      const dangerousPatterns = [/[<>\"']/, /javascript:/i, /<script/i];
  ...
  > => {
    try {
      ...
      if (request.method !== 'GET' && request.method !== 'HEAD') {
        ...
        if (!validateRequestSize(text, 10 * 1024 * 1024)) {
          // 10MB limit
          return {
            success: false,
