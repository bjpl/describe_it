...
export class APIMiddleware {
  ...
  async processRequest(
    ...
    try {
      ...
      if (!securityCheck.passed) {
        await monitor.recordMetrics(endpoint, request.method, 403, { reason: 'security' });
      ...
      if (!rateLimitCheck.allowed) {
        ...
        return NextResponse.json(
          ...
          { 
            status: 429,
            headers: {
              'X-RateLimit-Limit': ENDPOINT_CONFIGS[endpoint as keyof typeof ENDPOINT_CONFIGS]?.rateLimit.maxRequests.toString() || '100',
      ...
      if (!validationResult.valid) {
        await monitor.recordMetrics(endpoint, request.method, 400, { reason: 'validation' });
        return NextResponse.json(
  ...
  private async validateSecurity(request: NextRequest, endpoint: string): Promise<{
    passed: boolean;
    reason?: string;
  }> {
    // Check request size
    const config = ENDPOINT_CONFIGS[endpoint as keyof typeof ENDPOINT_CONFIGS];
    if (config && !InputValidator.validateRequestSize(request, config.maxRequestSize)) {
    ...
    if (isSuspicious && Math.random() > 0.8) { // Random blocking of 20% suspicious requests
  ...
  private async validateInput(request: NextRequest, endpoint: string): Promise<{
    valid: boolean;
    errors: any[];
  }> {
    const errors: any[] = [];
      ...
      if (['POST', 'PUT', 'PATCH'].includes(request.method)) {
        ...
        if (body) {
          ...
          ['language', 'fromLanguage', 'toLanguage'].forEach(field => {
            if (body[field] && !InputValidator.validateLanguageCode(body[field])) {
              errors.push({ 
                field, 
  ...
  private async postProcessResponse(
    ...
    newResponse.headers.set('X-Response-Time', monitor.getFormattedResponseTime());

    // Add rate limit headers
    const config = ENDPOINT_CONFIGS[endpoint as keyof typeof ENDPOINT_CONFIGS];
    if (config) {
      newResponse.headers.set('X-RateLimit-Limit', config.rateLimit.maxRequests.toString());
    ...
    if (origin) {
      ...
      newResponse.headers.set('Access-Control-Expose-Headers', 'X-Request-ID, X-Response-Time, X-Cache, X-RateLimit-Remaining');
    }
