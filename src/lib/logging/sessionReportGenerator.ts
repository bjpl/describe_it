...
export class SessionReportGenerator {
  ...
  public generateDetailedReport(): SessionReport {
    ...
    const recommendations = this.generateDetailedRecommendations(summary, learningMetrics);
  ...
  public generateTimeAnalysisReport(): {
    ...
    peakActivityPeriods: Array<{ start: number; end: number; intensity: number }>;
    ...
    interactions.forEach(interaction => {
      const windowStart = Math.floor(interaction.timestamp / windowSize) * windowSize;
      ...
      timeBreakdown[interaction.type] = (timeBreakdown[interaction.type] || 0) + estimatedDuration;
    });
    ...
    const activeTime = Object.values(timeBreakdown).reduce((sum, time) => sum + time, 0);
  ...
  public generateComparisonReport(previousSessions: SessionSummary[]): {
    ...
    // Compare key metrics
    const comparisons = [
      { metric: 'Learning Score', current: currentSummary.learningScore, previous: previousAvg.learningScore },
      { metric: 'Engagement Score', current: currentSummary.engagementScore, previous: previousAvg.engagementScore },
      { metric: 'Vocabulary Selected', current: currentSummary.vocabularySelected, previous: previousAvg.vocabularySelected },
      { metric: 'Descriptions Generated', current: currentSummary.descriptionsGenerated, previous: previousAvg.descriptionsGenerated },
      { metric: 'Questions Generated', current: currentSummary.questionsGenerated, previous: previousAvg.questionsGenerated }
    ];
    ...
    const overallTrend = improvementCount > totalComparisons / 2 ? 'improving' : 
                        improvementCount === 0 ? 'declining' : 'stable';
  ...
  public generateVocabularyReport(): {
    ...
    const interactions = this.sessionLogger.getInteractions();
    const vocabularyInteractions = interactions.filter(i => i.type === 'vocabulary_selected');
    ...
    const allCategories = ['objetos', 'acciones', 'lugares', 'colores', 'emociones', 'conceptos'];
  ...
  public generateErrorAnalysisReport(): {
    ...
    const errorInteractions = interactions.filter(i => i.type === 'error_occurred');
    ...
    for (let i = 0; i < timeWindows; i++) {
      ...
      const errorsInWindow = errorInteractions.filter(interaction =>
        interaction.timestamp >= windowStart && interaction.timestamp < windowEnd
      ).length;
    ...
    const stability = errorRate < 2 ? 'excellent' : 
                     errorRate < 5 ? 'good' : 
                     errorRate < 10 ? 'fair' : 'poor';
  ...
  private generateDetailedRecommendations(
    ...
    if (summary.learningScore < 30) {
      recommendations.push('Focus on exploring different description styles to improve learning variety');
      recommendations.push('Try generating questions about the images to test comprehension');
    } else if (summary.learningScore < 70) {
      recommendations.push('Great progress! Try selecting more vocabulary words to boost your score');
      recommendations.push('Challenge yourself with more complex images and descriptions');
    } else {
      recommendations.push('Excellent learning engagement! Consider helping others or exploring advanced features');
    }
    
    // Engagement recommendations
    if (summary.engagementScore < 50) {
      recommendations.push('Take breaks between sessions to maintain focus and retention');
      recommendations.push('Try shorter, more focused learning sessions');
    }
    
    // Activity-specific recommendations
    if (summary.descriptionsGenerated === 0) {
      recommendations.push('Start by generating image descriptions to practice language skills');
    }
    
    if (summary.vocabularySelected < 10) {
      recommendations.push('Select vocabulary words while reading descriptions to build your lexicon');
    }
    
    if (summary.questionsGenerated === 0) {
      recommendations.push('Use Q&A generation to test and reinforce your understanding');
    }
    
    // Error-based recommendations
    if (summary.errorCount > 0) {
      recommendations.push('Check your internet connection if you experience frequent errors');
    ...
    switch (summary.comprehensionLevel) {
      case 'beginner':
        recommendations.push('Start with simple, conversational description styles');
        recommendations.push('Focus on basic vocabulary categories like objects and colors');
        break;
      case 'intermediate':
        recommendations.push('Experiment with academic or poetic description styles');
        recommendations.push('Challenge yourself with abstract concepts and emotions');
        break;
      case 'advanced':
        recommendations.push('Try mixing different description styles in one session');
        recommendations.push('Create your own vocabulary categories and word groups');
        break;
    }
  ...
  private calculateMilestones(summary: SessionSummary): string[] {
    ...
    if (summary.totalInteractions >= 10) milestones.push('Active Learner (10+ interactions)');
    if (summary.descriptionsGenerated >= 5) milestones.push('Content Creator (5+ descriptions)');
    if (summary.vocabularySelected >= 20) milestones.push('Vocabulary Builder (20+ words)');
    if (summary.questionsGenerated >= 3) milestones.push('Critical Thinker (3+ questions)');
    if (summary.imagesViewed >= 10) milestones.push('Visual Explorer (10+ images)');
    if (summary.learningScore >= 70) milestones.push('High Achiever (70+ learning score)');
    if (summary.totalDuration >= 10 * 60 * 1000) milestones.push('Dedicated Learner (10+ minutes)');
  ...
  private calculateProgressToNext(summary: SessionSummary): number {
    ...
    switch (summary.comprehensionLevel) {
      ...
      case 'intermediate':
        ...
        const advVocabProgress = Math.min(summary.vocabularySelected / 50, 1) * 50;
  ...
  private calculateAverageMetrics(sessions: SessionSummary[]): SessionSummary {
    ...
    const average: Partial<SessionSummary> = {
      learningScore: sessions.reduce((sum, s) => sum + s.learningScore, 0) / count,
      engagementScore: sessions.reduce((sum, s) => sum + s.engagementScore, 0) / count,
      vocabularySelected: sessions.reduce((sum, s) => sum + s.vocabularySelected, 0) / count,
      descriptionsGenerated: sessions.reduce((sum, s) => sum + s.descriptionsGenerated, 0) / count,
      questionsGenerated: sessions.reduce((sum, s) => sum + s.questionsGenerated, 0) / count,
      totalInteractions: sessions.reduce((sum, s) => sum + s.totalInteractions, 0) / count,
      imagesViewed: sessions.reduce((sum, s) => sum + s.imagesViewed, 0) / count
    };
  ...
  private getErrorSuggestion(errorType: string): string {
    const suggestions: Record<string, string> = {
      ...
      'generation_failed': 'Generation failed - check image quality and try again',
  ...
  public generateProgressMetrics(interactions: SessionInteraction[], summary: SessionSummary): ProgressMetrics {
    const timeSpentByActivity: Record<string, number> = {};
    
    interactions.forEach(interaction => {
      const estimatedTime = this.estimateActivityDuration(interaction.type);
      timeSpentByActivity[interaction.type] = (timeSpentByActivity[interaction.type] || 0) + estimatedTime;
    });
  ...
  private calculateAccuracyTrend(interactions: SessionInteraction[]): Array<{ timestamp: number; accuracy: number; }> {
    const qaInteractions = interactions.filter(i => i.type === 'qa_generated' || i.type === 'qa_viewed');
    ...
    qaInteractions.forEach(interaction => {
      ...
      const baseAccuracy = difficulty === 'facil' ? 0.85 : difficulty === 'medio' ? 0.70 : 0.55;
  ...
  private calculateLearningCurve(interactions: SessionInteraction[]): Array<{ session: number; score: number; }> {
    const sessionChunks = this.chunkInteractionsByTime(interactions, 10 * 60 * 1000); // 10-minute sessions
  ...
  private calculateActivityHeatmap(interactions: SessionInteraction[]): Array<{ hour: number; activity: number; }> {
  ...
  private calculateVocabularyGrowth(interactions: SessionInteraction[]): Array<{ date: string; cumulative: number; new: number; }> {
  ...
  private calculateDifficultyProgression(interactions: SessionInteraction[]): Array<{ timestamp: number; difficulty: number; }> {
    ...
    return qaInteractions.map(interaction => {
      const difficulty = interaction.data.qaDifficulty;
      const difficultyValue = difficulty === 'facil' ? 1 : difficulty === 'medio' ? 2 : 3;
      return {
  ...
  private generateVocabularyGrowthChart(interactions: SessionInteraction[]): ChartData {
  ...
  private generateAccuracyTrendChart(interactions: SessionInteraction[]): ChartData {
    const accuracyTrend = this.calculateAccuracyTrend(interactions);
    
    return {
      labels: accuracyTrend.map(item => new Date(item.timestamp).toLocaleTimeString()),
      datasets: [{
        label: 'Accuracy %',
  ...
  private generateActivityHeatmap(interactions: SessionInteraction[]): Array<{ hour: number; count: number; }> {
    return this.calculateActivityHeatmap(interactions)
      .map(item => ({ hour: item.hour, count: item.activity }));
  }

  private generateLearningIntensityHeatmap(interactions: SessionInteraction[]): Array<{ period: string; intensity: number; }> {
  ...
  // Helper methods
  private chunkInteractionsByTime(interactions: SessionInteraction[], chunkSize: number): SessionInteraction[][] {
  ...
  private calculateChunkIntensity(chunk: SessionInteraction[]): number {
    const highValueInteractions = ['description_generated', 'qa_generated', 'vocabulary_selected'];
    const highValueCount = chunk.filter(i => highValueInteractions.includes(i.type)).length;
  ...
  // Export methods
  public exportReport(format: 'json' | 'text' | 'csv' | 'pdf' = 'json'): string {
  ...
  public async exportVisualReport(format: 'html' | 'pdf' = 'html'): Promise<string> {
    const report = this.generateDetailedReport();
...
Common Issues: ${errorAnalysis.commonErrors.map(e => e.type).slice(0, 3).join(', ')}
...
MILESTONES ACHIEVED
===================
${this.calculateMilestones(report.summary).map(m => `🏆 ${m}`).join('\n')}
...
Report generated by Describe It Learning Analytics
  ...
  private formatReportAsCSV(report: SessionReport): string {
    ...
    const mainData = [
      ['Metric', 'Value'],
      ['Session ID', report.summary.sessionId],
      ['Duration (minutes)', Math.round(report.summary.totalDuration / 1000 / 60).toString()],
      ...
      ['Descriptions Generated', report.summary.descriptionsGenerated.toString()],
  ...
  private generateHTMLReport(report: SessionReport, visualData: VisualReportData): string {
    const timeAnalysis = this.generateTimeAnalysisReport();
    const vocabularyReport = this.generateVocabularyReport();
...
<body>
    ...
    <div class="content">
      ...
      <div class="card">
        ...
        <div class="milestones">
          ${this.calculateMilestones(report.summary).map(m => `<div class="milestone">${m}</div>`).join('')}
        </div>
      </div>
    </div>
  </div>
  ...
  private async generateEnhancedPDFReport(report: SessionReport, visualData: VisualReportData): Promise<string> {
...
export function createReportGenerator(sessionLogger: SessionLogger): SessionReportGenerator {
