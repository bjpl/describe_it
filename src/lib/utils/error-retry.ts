...
const DEFAULT_RETRY_CONFIG: Required<RetryConfig> = {
  ...
  shouldRetry: (error: Error, attempt: number) => {
    ...
    const is5xxError = errorMessage.includes('50') || errorMessage.includes('server error');
    
    // Don't retry on 4xx client errors (except 429 rate limit)
    const is4xxError = errorMessage.includes('40') && !errorMessage.includes('429');
...
export async function withRetry<T>(
  ...
  for (let attempt = 0; attempt <= finalConfig.maxRetries; attempt++) {
    ...
    try {
      ...
      console.warn(`Attempt ${attempt + 1} failed, retrying in ${jitteredDelay}ms:`, lastError.message);
...
export async function apiCallWithRetry<T>(
  ...
  if (fallbackData !== undefined) {
    console.warn('API call failed, using fallback data:', result.error?.message);
    return fallbackData;
  }
...
export const RETRY_CONFIGS = {
  ...
  descriptionGeneration: {
    ...
    shouldRetry: (error: Error) => 
      !error.message.includes('400') && !error.message.includes('invalid image')
  },
