...
import { SessionReportGenerator, VisualReportData } from '@/lib/logging/sessionReportGenerator';
...
export class PDFExporter {
  ...
  private addExecutiveSummary(summary: SessionSummary): void {
    ...
    this.pdf.setFontSize(12);
    this.pdf.setFont('helvetica', 'normal');
    this.pdf.text(`Learning Score: ${summary.learningScore}/100`, this.margin, this.currentY);
    ...
    this.pdf.rect(this.margin + 60, this.currentY - 2, barWidth, barHeight, 'F');
    
    // Score fill color based on performance
    if (summary.learningScore >= 80) this.pdf.setFillColor(34, 197, 94); // green
    else if (summary.learningScore >= 60) this.pdf.setFillColor(59, 130, 246); // blue
    else if (summary.learningScore >= 40) this.pdf.setFillColor(245, 158, 11); // yellow
    else this.pdf.setFillColor(239, 68, 68); // red
    
    this.pdf.rect(this.margin + 60, this.currentY - 2, barWidth * scorePercent, barHeight, 'F');
    this.currentY += 10;
  ...
  private addKeyMetrics(summary: SessionSummary, reportGenerator: SessionReportGenerator): void {
    ...
    const metrics = [
      ...
      { label: 'Words Generated', value: summary.totalWordsGenerated.toString() },
      { label: 'Vocabulary Categories', value: vocabularyReport.categoriesExplored.length.toString() },
      { label: 'Time Efficiency', value: `${timeAnalysis.efficiencyScore}%` },
      { label: 'Error Rate', value: `${((summary.errorCount / summary.totalInteractions) * 100).toFixed(1)}%` },
      { label: 'Peak Activity Periods', value: timeAnalysis.peakActivityPeriods.length.toString() }
    ];
  ...
  private async addCharts(visualData: VisualReportData): Promise<void> {
    ...
    heatmapData.forEach(item => {
      ...
      // Activity bar
      const barWidth = (item.count / Math.max(...heatmapData.map(d => d.count))) * 60;
      this.pdf.setFillColor(139, 92, 246);
      this.pdf.rect(this.margin + 30, this.currentY - 2, barWidth, 3, 'F');
      
      this.pdf.text(`${item.count} activities`, this.margin + 100, this.currentY);
      this.currentY += 7;
    });
  ...
  private addDetailedAnalytics(report: SessionReport, reportGenerator: SessionReportGenerator): void {
    this.checkPageBreak(100);
    ...
    this.addSection('System Performance', [
      ...
      `Common error types: ${errorAnalysis.commonErrors.map(e => e.type).slice(0, 3).join(', ')}`
    ]);
  }
  ...
  private addRecommendations(recommendations: string[]): void {
    ...
    this.pdf.rect(this.margin, this.currentY - 5, this.pageWidth - 2 * this.margin, boxHeight, 'FD');

    this.pdf.setFontSize(11);
    this.pdf.setFont('helvetica', 'normal');
    recommendations.forEach((recommendation, index) => {
      this.pdf.text(`${index + 1}. ${recommendation}`, this.margin + 5, this.currentY + 5);
      this.currentY += 8;
    });
