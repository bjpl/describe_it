...
export class ExcelExporter {
  ...
  async exportToExcel(data: ExportData): Promise<Blob> {
    try {
      ...
      if (data.summary) {
        worksheets.push(
          this.prepareSummaryWorksheet(data.summary, data.metadata),
        );
      }
   ...
   */
  private prepareVocabularyWorksheet(
   ...
   */
  private prepareDescriptionsWorksheet(
   ...
   */
  private prepareSessionsWorksheet(
  ...
  private prepareSummaryWorksheet(summary: any, metadata: any): WorksheetData {
    const data = [
      ...
      {
        Metric: "Intermediate Words",
        Value: summary.progress?.intermediateWords || 0,
      },
  ...
  private createWorksheet(worksheetData: WorksheetData): void {
    ...
    if (worksheetData.format === "table") {
      ...
      if (this.options.formatting?.headers) {
        this.formatHeaders(
          worksheet,
          worksheetData.headers || Object.keys(worksheetData.data[0] || {}),
        );
      }
    } else {
    ...
    if (
      this.options.formatting?.freezePanes &&
      worksheetData.format === "table"
    ) {
    ...
    if (
      worksheetData.name === "Vocabulary" &&
      this.options.conditional?.difficultyColors
    ) {
      ...
      dateRange: {
        start:
          vocabularyData.length > 0
            ? vocabularyData[0].dateAdded
            : new Date().toISOString(),
        end: new Date().toISOString(),
      },
      categories: {},
      progress: {
        beginnerWords: vocabularyData.filter((v) => v.difficulty === "beginner")
          .length,
        intermediateWords: vocabularyData.filter(
          (v) => v.difficulty === "intermediate",
        ).length,
        advancedWords: vocabularyData.filter((v) => v.difficulty === "advanced")
          .length,
      },
    },
  };
