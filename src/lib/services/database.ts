...
export class DatabaseService {
  ...
  private async executeWithRetry<T>(
    ...
    for (let attempt = 1; attempt <= this.config.retryAttempts!; attempt++) {
      try {
        ...
        this.log(
          `Operation ${operationName} failed (attempt ${attempt}/${this.config.retryAttempts}): ${error}`,
          "warn",
        );
    ...
    return {
      ...
      cacheHitRate:
        this.queryCache.size > 0
          ? Math.round(
              (this.metrics.totalQueries / this.queryCache.size) * 100,
            ) / 100
          : 0,
    };
  }
        ...
        .insert([
          {
            ...
            default_description_style:
              userData.default_description_style || "conversacional",
  ...
  async getUserSessions(
    ...
    const cacheKey = this.createCacheKey("sessions", "getUserSessions", {
      userId,
      options,
    });
  ...
  async getVocabularyLists(
    options: QueryOptions = {},
  ): Promise<DatabaseResult<VocabularyList[]>> {
    ...
    return this.executeWithRetry(async () => {
      let query = this.supabase.from("vocabulary_lists").select("*");
  ...
  async createVocabularyList(
    listData: Partial<VocabularyList>,
  ): Promise<DatabaseResult<VocabularyList>> {
    return this.executeWithRetry(async () => {
      const { data, error } = await this.supabase
  ...
  async getVocabularyItems(
    ...
    const cacheKey = this.createCacheKey("vocabulary_items", "getByList", {
      listId,
      options,
    });
  ...
  async addVocabularyItem(
    itemData: Partial<VocabularyItem>,
  ): Promise<DatabaseResult<VocabularyItem>> {
    return this.executeWithRetry(async () => {
      const { data, error } = await this.supabase
        .from("vocabulary_items")
  ...
  async getLearningProgress(
    ...
    const cacheKey = this.createCacheKey("learning_progress", "getByUser", {
      userId,
      options,
    });
  ...
  async getQAResponses(
    ...
    const cacheKey = this.createCacheKey("qa_responses", "get", {
      userId,
      options,
    });
    ...
    return this.executeWithRetry(async () => {
      let query = this.supabase.from("qa_responses").select("*");
  ...
  async saveDescription(
    descriptionData: Partial<SavedDescription>,
  ...
  async getSavedDescriptions(
    ...
    const cacheKey = this.createCacheKey("saved_descriptions", "get", {
      userId,
      options,
    });
  ...
  async searchVocabulary(
    ...
    return this.executeWithRetry(async () => {
      let query = this.supabase
        ...
        .select("*")
        .or(
          `spanish_text.ilike.%${searchTerm}%,english_translation.ilike.%${searchTerm}%`,
        );
      ...
      const analytics = {
        sessions: {
          ...
          avgAccuracy: sessions?.length
            ? (sessions.reduce(
                (sum, s) =>
                  sum + (s.qa_correct || 0) / Math.max(s.qa_attempts || 1, 1),
                0,
              ) /
                sessions.length) *
              100
        ...
        vocabulary: {
          learned:
            progress?.filter((p) => p.learning_phase !== "new").length || 0,
          mastered:
            progress?.filter((p) => p.learning_phase === "mastered").length ||
            0,
          avgMastery: progress?.length
            ? progress.reduce((sum, p) => sum + p.mastery_level, 0) /
              progress.length
            : 0,
        },
        dateRange,
