...
export class VocabularyStorage {
  ...
  static updateVocabularySet(updatedSet: VocabularySet): boolean {
    const data = this.loadData();
    const index = data.vocabularySets.findIndex(
  ...
  static getStudyHistory(days: number = 30): StudySession[] {
    ...
    return data.studyHistory.filter(
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (values.length >= 6) {
          ...
          const phrase: SavedPhrase = {
            ...
            difficulty:
              (values[5] as "beginner" | "intermediate" | "advanced") ||
              "beginner",
            context: values[6] || "",
            gender:
